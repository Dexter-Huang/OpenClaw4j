...
interface RequestConfig {
  ...
  timeout?: number;
  skipInterceptors?: boolean; // 是否跳过拦截�?
}
  ...
  async processError(error: RequestError): Promise<never> {
    ...
    for (const interceptor of this.interceptors) {
      if (interceptor && interceptor.responseError) {
        try {
          throw interceptor.responseError(currentError);
        } catch (processedError) {
          // 如果拦截器返回了新的错误，继续抛�?
          currentError =
            processedError instanceof RequestError
              ? processedError
              : currentError;
        }
      }
...
async function baseRequest<T = any>(
  ...
  try {
    ...
    const { params, data, headers = {} } = processedConfig;
    ...
    if (!response.ok) {
      ...
      console.error(
        `[REQUEST ERROR] ${method} ${fullUrl} - ${response.status}`,
        errorText,
      );
