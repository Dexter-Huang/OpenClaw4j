...
const getEvaluatorModelName = (
  ...
  try {
    ...
    return modelNameMap[parsedModelConfig?.modelId];
  } catch (error) {
    console.warn('Failed to parse evaluator modelConfig:', error);
    return undefined;
  }
};
...
const EvaluationEvaluator: React.FC = () => {
  ...
  const { pagination, onPaginationChange, onShowSizeChange, setPagination } =
    usePagination();
  ...
  const handleDelete = (record: EvaluatorRecord) => {
    Modal.confirm({
      ...
      content: (
        <div>
          <p>
            确定要删除评估器 <strong>{record.name}</strong> 吗？
          </p>
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (modelConfig?: string) => {
        ...
        const name = getEvaluatorModelName(modelConfig, modelNameMap);
