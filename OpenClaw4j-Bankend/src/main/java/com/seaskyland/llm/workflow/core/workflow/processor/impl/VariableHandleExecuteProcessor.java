...
public class VariableHandleExecuteProcessor extends AbstractExecuteProcessor {
  ...
  public VariableHandleExecuteProcessor(
  ...
  /**
   * Executes the variable handling logic for the node Processes the node based on its type
   * (template, json, or group)
   *
  ...
  public NodeResult innerExecute(
    ...
    if (nodeParam.getType().equals(TextConvertorType.template.name())) {
      ...
      nodeResult.setOutput(
          JsonUtils.toJson(constructJsonResult(nodeParam.getJsonParams(), context)));
  ...
  /**
   * Constructs a result map for group-based variable handling Groups variables and selects values
  ...
  private void handleGroupVariables(
      NodeParam nodeParam, Map<String, Object> groupVariableMap, WorkflowContext context) {
              ...
              for (CommonParam variable : variables) {
                ...
                if (valueFromContext != null) {
                  ...
                  if (nodeParam.getGroupStrategy() == null
                      || nodeParam
                          .getGroupStrategy()
                          .equals(GroupStrategyEnum.firstNotNull.name())) {
                    break;
                  }
                }
              }
  ...
  /**
   * Method to construct JSON result Based on input parameters and context information, constructs a
   * JSONObject Mainly used to handle values from references or direct assignments, and integrate
   * them into a JSON object
   *
   * @param jsonParams Input parameter list, containing parameter keys, values, and value sources
  ...
  public CheckNodeParamResult checkNodeParam(DirectedAcyclicGraph<String, Edge> graph, Node node) {
    ...
    if (nodeParam.getType().equals(TextConvertorType.template.name())
      ...
      result.getErrorInfos().add("Text content is empty");
        ...
        && nodeParam.getJsonParams().isEmpty()) {
      result.setSuccess(false);
      result.getErrorInfos().add("No variables added");
    } else if (nodeParam.getType().equals(TextConvertorType.group.name())
        && nodeParam.getGroups().isEmpty()) {
      result.setSuccess(false);
