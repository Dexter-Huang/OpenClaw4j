...
import com.seaskyland.llm.workflow.core.rag.advisor.KnowledgeBaseRetrievalAdvisor;
import com.seaskyland.llm.workflow.core.utils.common.IdGenerator;
import com.seaskyland.llm.workflow.core.utils.io.FileUtils;
...
import com.seaskyland.llm.workflow.runtime.domain.chat.MultimodalContent;
 ...
 * functionality for both streaming and non-streaming agent responses.
 *
 * @since 1.0.0.3
 */
@Service()
...
public class BasicAgentExecutor extends AbstractAgentExecutor {
  ...
  private OpenAiChatOptions buildChatOptions(
    ...
    if (config.getParameter() != null) {
      builder
          .maxTokens(config.getParameter().getMaxTokens())
  ...
  private ChatClient.Builder buildChatClient(
    ...
    if (context.isMemoryEnabled()) {
      ...
      String conversationId =
          String.format("%s_%s", context.getAppId(), request.getConversationId());
      // chatClientBuilder
      ...
      chatClientBuilder.defaultAdvisors(
          memoryAdvisor -> memoryAdvisor.param(CONVERSATION_ID, conversationId));
    }

    // Add document retriever
    FileSearchOptions fileSearchOptions = config.getFileSearch();
    if (fileSearchOptions != null
        && fileSearchOptions.getEnableSearch()
        && !CollectionUtils.isEmpty(fileSearchOptions.getKbIds())) {
      DocumentRetriever documentRetriever =
          documentRetrieverManager.getDocumentRetriever(fileSearchOptions);
      Advisor retrievalAdvisor =
          ...
          if (chatMessage.getContentType() == ContentType.TEXT) {
            ...
            throw new BizException(
                ErrorCode.INVALID_PARAMS.toError(
                    "content_type", chatMessage.getContentType().getValue() + "not supported"));
          }
        }
   ...
   */
  protected Mono<AgentResponse> convertResponse(
      ChatResponse chatResponse, CompositeToolCallbackProvider toolCallbackProvider) {
    ...
    AgentResponse.AgentResponseBuilder responseBuilder =
        AgentResponse.builder().model(chatResponse.getMetadata().getModel());
    ...
    if (!CollectionUtils.isEmpty(chatResponse.getResults())) {
      ...
      String reasoningContent =
          reasoningContentObj == null ? null : String.valueOf(reasoningContentObj);
      reasoningContent = StringUtils.isBlank(reasoningContent) ? null : reasoningContent;

      ChatMessage.ChatMessageBuilder messageBuilder =
          ChatMessage.builder()
      ...
      if (!CollectionUtils.isEmpty(generation.getOutput().getToolCalls())) {
        toolCalls.addAll(
            convertToolCall(generation.getOutput().getToolCalls(), toolCallbackProvider));
      }
   ...
   */
  private List<ToolCall> convertToolCall(
              ...
              if (toolCallback != null) {
                ...
                Map<String, Object> argumentsMap =
                    ToolArgumentsHelper.mergeToolArguments(
                        toolCall.arguments(),
                        toolCallbackProvider.getExtraParams(),
                        toolCallback.getId());
    ...
    if (metadata.containsKey(FILE_SEARCH_CALL)) {
      ToolCall toolCall =
          ToolCall.builder()
              ...
              .function(
                  ToolCall.Function.builder()
                      .arguments(JsonUtils.toJson(metadata.get(FILE_SEARCH_CALL)))
                      .build())
              .build();
   ...
   * @return List of tool calls
   */
  private List<ToolCall> convertToolResult(
      ToolExecutionResult toolExecutionResult, CompositeToolCallbackProvider toolCallbackProvider) {
    List<ToolCall> toolResults = new ArrayList<>();
    List<Message> conversationHistory = toolExecutionResult.conversationHistory();

    Map<String, AgentToolCallback> toolCallbackMap = getAgentToolCallback(toolCallbackProvider);

    if (conversationHistory.get(conversationHistory.size() - 1)
      ...
      toolResponseMessage
          ...
          .forEach(
              response -> {
                ...
                ToolCallType type =
                    toolCallback == null
                        ? ToolCallType.TOOL_RESULT
                        : toolCallback.getToolCallType();
   ...
   * @return Agent response
   */
  protected AgentResponse convertToolResult(
   ...
   */
  private CompositeToolCallbackProvider buildToolCallbackProvider(
      AgentConfig config, Map<String, Object> extraParams) {
    return new CompositeToolCallbackProvider(
        config,
        pluginService,
            ...
            .flatMap(
                array -> {
                  ...
                  return Flux.concat(
                      ...
                      // calls
                      chatClientBuilder.build().prompt(newPrompt).stream()
                          .chatResponse()
   ...
   */
  private Map<String, AgentToolCallback> getAgentToolCallback(
      ToolCallbackProvider toolCallbackProvider) {
