...
/**
 * Factory class for creating various AI model instances including chat, embedding, and document
 * ranking models.
 *
 * @since 1.0.0.3
 */
...
public class ModelFactory {
  ...
  @Resource private Map<String, ModelProvider> providerMap;
  ...
  public EmbeddingModel getEmbeddingModel(MetadataMode metadataMode, IndexConfig indexConfig) {
    ModelCredential credential =
        getModelCredential(indexConfig.getEmbeddingProvider(), indexConfig.getEmbeddingModel());

    int dimension =
        EmbeddingModelDimension.getDimension(indexConfig.getEmbeddingModel(), DEFAULT_DIMENSION);

    return OpenAiEmbeddingModel.builder()
        .metadataMode(metadataMode)
        .options(
            buildOpenAiEmbeddingOptions(credential, indexConfig.getEmbeddingModel(), dimension))
        .build();
  }
   ...
   */
  private OpenAiEmbeddingOptions buildOpenAiEmbeddingOptions(
      ModelCredential credential, String modelId, int dimension) {
    OpenAiEmbeddingOptions.Builder builder =
    ...
    if (StringUtils.isNotBlank(credential.getEndpoint())) {
      builder.baseUrl(credential.getEndpoint());
      log.debug(
          "Built OpenAI embedding options - endpoint: {}, model: {}",
