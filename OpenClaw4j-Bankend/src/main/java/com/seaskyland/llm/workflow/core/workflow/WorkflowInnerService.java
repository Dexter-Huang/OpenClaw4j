...
import com.seaskyland.llm.workflow.core.workflow.processor.impl.OutputExecuteProcessor;
...
/**
 * Internal service for workflow management. This service handles workflow execution status tracking
 * and context caching. It provides functionality for: - Managing workflow task execution flags -
 * Caching workflow context - Determining caching strategy based on invocation source
 *
 * @since 1.0.0.3
 */
@Slf4j
...
public class WorkflowInnerService {
  ...
  private static final Set<String> CAN_STREAM_NODE_TYPE_SET =
      Sets.newHashSet(NodeTypeEnum.LLM.getCode(), NodeTypeEnum.COMPONENT.getCode());
   ...
   *
   * @param cacheManager Redis manager for cache operations
   */
  public WorkflowInnerService(CacheManager cacheManager) {
    this.cacheManager = cacheManager;
  }

  public WorkflowContext getContextCache(String workspaceId, String taskId) {
    WorkflowContext context =
        cacheManager.get(WORKFLOW_TASK_CONTEXT_PREFIX + workspaceId + "_" + taskId);
  ...
  /**
   * Updates the workflow context in cache with version conflict detection and merging. Only console
   * invocations require caching, API calls do not need cache refresh.
   *
  ...
  /**
   * Forces the update of workflow context in cache with version conflict detection and merging.
   * This method bypasses the invocation source check and always updates the cache. It retrieves the
   * latest context from cache, compares versions, and merges if necessary before saving.
   *
   * @param context WorkflowContext to be cached
   */
  public void forceRefreshContextCache(WorkflowContext context) {
    ...
    // 获取缓存中的最新context
    WorkflowContext existingContext = cacheManager.get(cacheKey);
    ...
    if (existingContext != null) {
      ...
      if (existingContext.getVersion() > context.getVersion()) {
        // 缓存中的版本更新或相同，需要合并
        log.debug(
            "Version conflict detected: existing={}, new={}, taskId={}",
            existingContext.getVersion(),
            context.getVersion(),
            context.getTaskId());
        ...
        // 保存合并后的context
        cacheManager.put(cacheKey, mergedContext, Duration.ofHours(1));

        log.debug(
            "Context merged and saved: taskId={}, newVersion={}",
            mergedContext.getTaskId(),
            mergedContext.getVersion());
      } else {
        // 新context版本更高，直接保存
        context.setVersion(existingContext.getVersion() + 1);
        cacheManager.put(cacheKey, context, Duration.ofHours(1));

        log.debug(
            "New context saved directly: taskId={}, newVersion={}",
            context.getTaskId(),
            context.getVersion());
      }
    } else {
      ...
      log.debug(
          "Initial context saved: taskId={}, version={}",
          context.getTaskId(),
          context.getVersion());
    }
  }
  ...
  /**
   * Determines if Redis caching is necessary based on the invocation source. Currently, only
   * console invocations require caching.
   *
  ...
  public boolean canExecute(
    ...
    if (NodeTypeEnum.OUTPUT.getCode().equals(nodeType)) {
      ...
      OutputExecuteProcessor.NodeParam config =
          JsonUtils.fromMap(
              node.getConfig().getNodeParam(), OutputExecuteProcessor.NodeParam.class);
    ...
    if (NodeTypeEnum.END.getCode().equals(nodeType)) {
      Node node = nodeOptional.get();
      EndExecuteProcessor.NodeParam config =
          JsonUtils.fromMap(node.getConfig().getNodeParam(), EndExecuteProcessor.NodeParam.class);
  ...
  /**
   * Checks if a streaming output node can be executed Considers the state of source nodes and
   ...
   * @return true if the node can be executed, false otherwise
   */
  private boolean checkStreamOutputNodeExecutable(
      Set<Edge> incomingEdges, WorkflowContext context, Set<String> keyFromSet) {
  ...
  /**
   * Checks if a common node can be executed Verifies that all incoming nodes have completed
   * successfully
   *
   * @param incomingEdges The incoming edges to the node
   * @param context The execution context
   ...
   * @return 合并后的context
   */
  private WorkflowContext mergeContextVersions(
      WorkflowContext existingContext, WorkflowContext newContext) {
    // 创建合并后的context，基于新context
    WorkflowContext mergedContext = WorkflowContext.deepCopy(newContext);

    if (mergedContext == null) {
      log.warn(
          "Failed to deep copy new context, using original: taskId={}", newContext.getTaskId());
      return newContext;
    }
      ...
      existingContext
          ...
          .forEach(
              (nodeId, nodeResult) -> {
                ...
                if (newResult == null
                    || isNodeStatusOlder(newResult.getNodeStatus(), nodeResult.getNodeStatus())) {
    ...
    if (existingContext.getExecuteOrderList() != null
        && mergedContext.getExecuteOrderList() != null) {
      // 合并执行顺序，避免重复
      existingContext
          .getExecuteOrderList()
          .forEach(
    ...
    log.debug(
        "Merged context versions: taskId={}, existingNodes={}, newNodes={}, mergedNodes={}",
        mergedContext.getTaskId(),
  ...
  private boolean isNodeStatusOlder(String newStatus, String oldStatus) {
    ...
    Map<String, Integer> statusPriority =
        Map.of(
            NodeStatusEnum.EXECUTING.getCode(),
            1,
            NodeStatusEnum.SUCCESS.getCode(),
            2,
            NodeStatusEnum.FAIL.getCode(),
            3,
            NodeStatusEnum.PAUSE.getCode(),
            4);
