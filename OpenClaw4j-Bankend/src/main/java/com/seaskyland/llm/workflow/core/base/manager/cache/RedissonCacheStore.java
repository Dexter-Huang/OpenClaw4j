 ...
 * <p>
 * Keys received by every method are expected to be fully qualified (prefix already
 * applied by the caller – typically {@link CacheManager}).
 *
 * @since 1.0.0.3
 */
@Slf4j
public class RedissonCacheStore implements CacheStore {
	...
	public boolean lock(String key, Duration duration) {
		...
		if (currentExpireTimeStr != null) {
			...
			if (currentExpireTime < now) {
				String lastExpireTimeStr = (String) redissonClient.getBucket(key).getAndSet(expireTimeStr, duration);
	...
	/**
	 * Returns the underlying Redisson {@link RSet} for the given key.
	 * Only available in Redis mode; callers should check via {@code instanceof RedissonCacheStore}.
	 */
