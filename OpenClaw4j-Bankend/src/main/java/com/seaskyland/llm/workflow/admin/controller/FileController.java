...
import com.seaskyland.llm.workflow.runtime.domain.file.WebUploadPolicy;
import com.seaskyland.llm.workflow.runtime.domain.file.WebUploadRequest;
...
public class FileController {
  ...
  public Result<List<UploadPolicy>> uploadDocument(
      @RequestPart("files") MultipartFile[] files, @RequestPart("category") String category) {
  ...
  public void downloadFile(
    ...
    if (preview) {
      response.setContentType(
          MediaType.parseMediaType(FileUtils.getContentType(filename)).toString());
  ...
  public Result<List<WebUploadPolicy>> getUploadPolicies(@RequestBody WebUploadRequest request) {
    ...
    for (WebUploadRequest.FileMeta fileMeta : request.getFiles()) {
      WebUploadPolicy policy =
          ossManager.getWebUploadPolicy(
              context.getAccountId(), request.getCategory(), fileMeta.getName());
      policies.add(policy);
    }
