...
import static com.seaskyland.llm.workflow.core.workflow.constants.WorkflowConstants.SYS_HISTORY_LIST_KEY;
import static com.seaskyland.llm.workflow.core.workflow.constants.WorkflowConstants.WORKFLOW_TASK_FINISH_FLAG;
...
import com.seaskyland.llm.workflow.core.workflow.processor.ExecuteProcessor;
import com.seaskyland.llm.workflow.core.workflow.processor.impl.ClassifierExecuteProcessor;
import com.seaskyland.llm.workflow.core.workflow.processor.impl.EndExecuteProcessor;
import com.seaskyland.llm.workflow.core.workflow.processor.impl.JudgeExecuteProcessor;
...
/**
 * WorkflowExecuteManager is responsible for managing the execution of workflow graphs. It handles
 * the construction and validation of workflow graphs, execution scheduling of nodes, monitoring of
 * workflow execution status, building of debug configurations, and checking of workflow
 * configurations. The manager ensures proper execution order, handles node dependencies, and
 * maintains execution state.
 *
 * <p>Key features: - Workflow graph construction and validation - Node execution scheduling and
 * monitoring - Execution state management - Debug configuration building - Workflow configuration
 * validation
 *
 * @since 1.0.0.3
 */
...
public class WorkflowExecuteManager {
  ...
  public WorkflowExecuteManager(
      @Lazy Map<String, AbstractExecuteProcessor> processorMap,
  ...
  public Boolean stopTask(String taskId) {
    ...
    WorkflowContext context =
        workflowInnerService.getContextCache(requestContext.getWorkspaceId(), taskId);
    if (context == null) {
      return false;
    }
            ...
            workflowContext
                ...
                .put(
                    ...
                    VariableUtils.convertValueByType(
                        input.getKey(), input.getType(), input.getValue()));
          } else {
            workflowContext
                .getUserMap()
                .put(
   ...
   *
   * @param applicationVersion The version of the application containing workflow configuration
  ...
  /**
   * Core execution method that processes the workflow Initializes execution context, sets up
   * history tracking, and submits execution task
   *
  ...
  public String execute(WorkflowContext context) {
    ...
    if (globalConfig != null
      ...
      historySwitch = true;
      historyMaxRound =
          globalConfig.getHistoryConfig().getHistoryMaxRound() == null
              ? 5
  ...
  /**
   * Synchronously executes the workflow Manages the execution queue, monitors node execution, and
   ...
   */
  public void syncExecute(WorkflowConfig appOrchestraConfig, WorkflowContext context)
    ...
    while (!needStop.get()) {
      ...
      if (nodeId != null && !WORKFLOW_TASK_FINISH_FLAG.equals(nodeId)) {
        ThreadPoolExecutor executor =
            ((RequestContextThreadPoolWrapper) ThreadPoolUtils.nodeExecutorService)
        ...
        if (executor != null) {
          ...
          log.info(
              "ThreadId:{} taskId:{} activeCount:{} taskCount:{} completedTaskCount:{} ",
    ...
    if (context.getTaskStatus().equals(NodeStatusEnum.SUCCESS.getCode())) {
      ...
      LogUtils.monitor(
          "WorkflowService", "runTask", context.getStartTime(), FAIL, appOrchestraConfig, context);
    }
  ...
  /**
   * Monitors the execution of workflow nodes Continuously checks for executable nodes and updates
   * the task queue
   *
    ...
    while (!done.get()) {
      ...
      for (String node : graph.vertexSet()) {
        ...
        if (b && !taskSet.contains(node)) {
          log.info(
              "nodeAnalysis canExecute:{}  taskID:{}  result:{}", node, context.getTaskId(), b);
          taskQueue.add(node);
          taskSet.add(node);
        }
      }
  ...
  private Integer getRefreshIntervalByInvokeSource(String invokeSource) {
    Map<Object, Object> workflowRefreshInterval =
        JsonUtils.fromJsonToMap(commonConfig.getWorkflowRefreshInterval());
  ...
  private DirectedAcyclicGraph<String, Edge> constructGraph(WorkflowConfig appOrchestraConfig) {
    ...
    appOrchestraConfig
        .getEdges()
        .forEach(edge -> graph.addEdge(edge.getSource(), edge.getTarget(), edge));
    return graph;
  }
  ...
  public DirectedAcyclicGraph<String, Edge> constructGraph(List<Node> nodes, List<Edge> edges) {
    ...
    if (sinkNodes.isEmpty()) {
      throw new BizException(
          ErrorCode.WORKFLOW_CONFIG_INVALID.toError(
              "Workflow must contain at least one end node with out-degree 0"));
    }
    ...
    if (!reachableFromSource.equals(graph.vertexSet())) {
      throw new BizException(
          ErrorCode.WORKFLOW_CONFIG_INVALID.toError(
              "There are nodes that cannot be reached from start nodes"));
    }
  ...
  private static Set<String> findReachableNodes(
      DirectedAcyclicGraph<String, Edge> graph, String startNode, boolean reverse) {
  ...
  private static Set<String> findReachableNodes(
      DirectedAcyclicGraph<String, Edge> graph, String startNode) {
    return findReachableNodes(graph, startNode, false);
  }

  /**
   * Checks if the workflow execution should stop Considers execution status, end node state, and
   * timeout conditions
   *
   * @param graph The workflow graph
   * @param context The execution context
  ...
  private boolean needStop(DirectedAcyclicGraph<String, Edge> graph, WorkflowContext context) {
    ...
    Optional<String> endNodeOptional =
        graph.vertexSet().stream()
            .filter(
                node ->
                    node.startsWith("End_")
                        || node.startsWith("IteratorEnd_")
    ...
    return containFail
        || (endNodeResult != null
            && endNodeResult.getNodeStatus().equals(NodeStatusEnum.SUCCESS.getCode()));
  }

  /**
   * Executes the work for a specific node Handles node state management, execution, and error
   * handling
   *
   * @param graph The workflow graph
  ...
  private void executeNodeWork(
      DirectedAcyclicGraph<String, Edge> graph, String nodeId, WorkflowContext context) {
    try {
      ...
      try {
        ...
        Optional<Node> any =
            context.getWorkflowConfig().getNodes().stream()
      ...
      log.info(
          "ThreadId:{} requestId:{} taskId:{} executeNodeWork monitor nodeId :{} NodeResultMap:{}",
          Thread.currentThread().getId(),
            ...
            incomingEdges.stream()
                .allMatch(
                    incomingEdge -> {
                      ...
                      if (result.getNodeStatus().equals(NodeStatusEnum.SUCCESS.getCode())
                          && result.isMultiBranch()) {
                        List<NodeResult.MultiBranchReference> multiBranchResults =
                            result.getMultiBranchResults();
                        if (CollectionUtils.isEmpty(multiBranchResults)) {
                          return true;
                        } else {
                          Optional<NodeResult.MultiBranchReference> referenceOptional =
                              multiBranchResults.stream()
                                  .filter(
      ...
      workflowInnerService.refreshContextCache(context);
      throw new BizException(
          ErrorCode.WORKFLOW_EXECUTE_ERROR.toError("error node id is: " + nodeId));
    }
  }
  ...
  /**
   * Constructs a debug configuration for a workflow fragment Adds necessary start and end nodes for
   * debugging
   *
   * @param nodes List of workflow nodes
   * @param edges List of workflow edges
  ...
  public WorkflowConfig constructDebugConfig4Fragment(List<Node> nodes, List<Edge> edges) {
    ...
    for (String sinkNode : sinkNodes) {
      ...
      if (node.getType().equals(NodeTypeEnum.CLASSIFIER.getCode())) {
        // classifier
        ClassifierExecuteProcessor.NodeParam nodeParam =
            JsonUtils.fromMap(
        ...
        // judge
        JudgeExecuteProcessor.NodeParam nodeParam =
            JsonUtils.fromMap(
  ...
  /**
   * Constructs debug variables from input map Converts flat key-value pairs into nested structure
   *
  ...
  /**
   * Validates the workflow configuration Checks node parameters and overall flow validity
   *
   * @param appOrchestraConfig The workflow configuration to check
   * @return Result of the configuration check
   */
  public ExecuteProcessor.CheckFlowParamResult checkWorkflowConfig(
