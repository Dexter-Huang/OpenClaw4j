...
/**
 * A document retriever that searches across multiple vector stores. It supports parallel retrieval
 * from different knowledge bases and optional document reranking.
 *
 * @since 1.0.0.3
 */
...
public class KnowledgeBaseDocumentRetriever implements DocumentRetriever {
  ...
  /**
   * Retrieves relevant documents from all knowledge bases based on the query. Documents are
   * retrieved in parallel and then merged, sorted, and filtered.
   *
    ...
    for (KnowledgeBase knowledgeBase : knowledgeBases) {
      CompletableFuture<List<Document>> textFuture =
          CompletableFuture.supplyAsync(
              () -> retrieve(knowledgeBase, query), ThreadPoolUtils.DEFAULT_TASK_EXECUTOR);
    ...
    try {
      ...
      if (searchOptions.getSimilarityThreshold() == null) {
        throw new BizException(
            ErrorCode.MISSING_PARAMS.toError("similarityThreshold"),
            "请设置相似度阈值参数 similarityThreshold");
      }
      ...
      LogUtils.monitor(
          "DocumentRetriever", "retrieve", start, SUCCESS, query.text(), results.size());
      return results;
  ...
  private List<Document> retrieve(KnowledgeBase knowledgeBase, Query query) {
    ...
    var exp =
        b.and(b.eq(KEY_WORKSPACE_ID, knowledgeBase.getWorkspaceId()), b.eq(KEY_ENABLED, true))
            .build();
    ...
    SearchRequest.Builder searchRequestBuilder =
        SearchRequest.builder().query(query.text()).filterExpression(exp).searchType(searchType);
    if (searchOptions.getSimilarityThreshold() != null) {
