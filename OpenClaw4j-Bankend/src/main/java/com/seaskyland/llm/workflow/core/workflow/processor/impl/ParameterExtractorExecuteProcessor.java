 ...
 *
 * <p>This processor is responsible for extracting structured parameters from unstructured text
 * input using LLM models. It provides a flexible and accurate way to parse user input into
 * predefined parameter formats.
 *
 * <p>Key Features: 1. Extracts parameters from natural language input using LLM 2. Supports custom
 * parameter definitions with types and descriptions 3. Handles parameter validation and completion
 * status 4. Provides example-based learning for better extraction 5. Supports short-term memory for
 * context-aware extraction 6. Configurable model parameters and instructions 7. JSON-formatted
 * output with completion status 8. Error handling and parameter validation
 *
 * @version 1.0.0-M1
 */
...
public class ParameterExtractorExecuteProcessor extends AbstractExecuteProcessor {
  ...
  private static final String SYSTEM_PROMPT =
      "You are a professional parameter extraction assistant. Your task is to accurately extract specified parameters from user input. Please carefully analyze the input content to ensure accurate parameter extraction. If certain parameters cannot be extracted, set _is_completed to false and explain the reason in _reason.";

  private static final String INSTRUCTION_PROMPT =
      "Please note the following content, which is crucial for parameter extraction: %s";

  private static final String PARAMETER_LIST_TEMPLATE =
      "Parameters to extract are in the format - {parameter_name}:{parameter_type}:{parameter_description}:{is_required}. The list of parameters to extract is:\n%s\n";
  ...
  public ParameterExtractorExecuteProcessor(
  ...
  public NodeResult innerExecute(
    ...
    promptBuilder.append(
        String.format(PARAMETER_LIST_TEMPLATE, buildParameterList(config.getExtractParams())));
    promptBuilder.append(EXAMPLES_TEMPLATE);
    promptBuilder.append(String.format(USER_INPUT_TEMPLATE, query));
    promptBuilder.append(
        String.format(JSON_TEMPLATE, buildJsonTemplate(config.getExtractParams())));
            ...
            () -> {
              try {
                ...
                Map<String, Object> extractedParams =
                    JsonUtils.fromJsonToMap(responseBuilder.toString());
