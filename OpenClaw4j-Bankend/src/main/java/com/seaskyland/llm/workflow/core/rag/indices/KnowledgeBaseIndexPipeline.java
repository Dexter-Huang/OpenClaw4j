...
import com.seaskyland.llm.workflow.core.rag.reader.TextDocumentReader;
import com.seaskyland.llm.workflow.core.rag.splitter.RegexTextSplitter;
...
/**
 * Pipeline for processing and indexing knowledge base documents. Handles document parsing,
 * transformation, and storage in vector store.
 *
 * @since 1.0.0.3
 */
@Service
...
public class KnowledgeBaseIndexPipeline implements IndexPipeline {
  ...
  public List<Document> parse(
      com.seaskyland.llm.workflow.runtime.domain.knowledgebase.Document document) {
      ...
      case "md", "markdown":
        {
          DocumentReader reader =
              new MarkdownDocumentReader(
                  new FileSystemResource(file), MarkdownDocumentReaderConfig.defaultConfig());
          documents = reader.get();
          break;
        }
      case "txt":
  ...
  public void store(List<Document> chunks, IndexConfig indexConfig, Map<String, Object> metadata) {
    ...
    log.info(
        "embedding and save to vector store, chunks: {}, indexConfig: {}",
        chunks.size(),
        indexConfig);
    chunks.forEach(
        chunk -> {
    ...
    VectorStore vectorStore =
        vectorStoreFactory.getVectorStoreService().getVectorStore(indexConfig);
