...
import com.seaskyland.llm.workflow.runtime.domain.component.CustomParam;
...
public class AppComponentManager {
  ...
  public AppComponentConfig getApplicationInputConfig(Application application, Boolean typeFilter) {
    ...
    if (application.getType().equals(AppType.BASIC)) {
      AgentConfig agentConfig =
          JsonUtils.fromJson(application.getPubConfigStr(), AgentConfig.class);
      ...
      WorkflowConfig appOrchestraConfig =
          JsonUtils.fromJson(application.getPubConfigStr(), WorkflowConfig.class);
      List<Node> start =
      ...
      EndExecuteProcessor.NodeParam nodeParam =
          JsonUtils.fromMap(
      ...
      if (appComponent == null) {
        throw new BizException(
            ErrorCode.INVALID_PARAMS.toError(
                "component_code", "can not find published component: " + code));
      }
   ...
   */
  public AppComponentConfig mergeConfig(
      AppComponentConfig applicationConfig, AppComponentConfig appComponentConfig) {
   ...
   */
  public Map<String, List<AppComponentConfig.Params>> fetchInputAndOutputParams(
      AppComponent appComponent) {
    ...
    if (input != null) {
      ...
      if (CollectionUtils.isNotEmpty(systemParams)) {
        ...
        inputParams.addAll(
            systemParams.stream()
                .filter(param -> BooleanUtils.isTrue(param.getDisplay()))
                .toList());
      }
   ...
   * @return List of custom parameters
   */
  public List<CustomParam> queryComponentParams(
      List<String> ComponentCodeList, Boolean typeFilter) {

    List<CustomParam> customParamDTOS = Lists.newArrayList();
    List<AppComponent> appComponentDTOs =
        appComponentService.getAppComponentListByCodes(ComponentCodeList);
    if (appComponentDTOs == null) {
      return customParamDTOS;
    }
    // get component config
    for (AppComponent appComponent : appComponentDTOs) {
      Map<String, List<AppComponentConfig.Params>> AppComponentConfigMap =
          fetchInputAndOutputParams(appComponent);
