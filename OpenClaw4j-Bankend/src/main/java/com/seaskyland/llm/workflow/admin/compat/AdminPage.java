...
public class AdminPage<T> {
  ...
  public static <T> AdminPage<T> of(
      long totalCount, long pageNumber, long pageSize, List<T> pageItems) {
    long safePageSize = pageSize <= 0 ? 10 : pageSize;
    long totalPage = totalCount == 0 ? 0 : (totalCount + safePageSize - 1) / safePageSize;
    return new AdminPage<>(
        totalCount, totalPage, pageNumber <= 0 ? 1 : pageNumber, safePageSize, pageItems);
  }
}
