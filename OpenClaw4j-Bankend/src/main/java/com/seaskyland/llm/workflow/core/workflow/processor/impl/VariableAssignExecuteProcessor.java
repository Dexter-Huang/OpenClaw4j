...
/**
 * Processor for variable assignment operations in workflows Handles the assignment of values
 * between variables
 *
 * @version 1.0.0-beta
 */
...
public class VariableAssignExecuteProcessor extends AbstractExecuteProcessor {
  ...
  public VariableAssignExecuteProcessor(
  ...
  /**
   * Executes the variable assignment operation Processes pairs of variables (left and right) and
  ...
  public NodeResult innerExecute(
    ...
    try {
      ...
      for (Pair pair : inputs) {
        ...
        object.put(
            "left",
            getValueFromPayload(leftExpression, context.getVariablesMap()) == null
                ? ""
                : getValueFromPayload(leftExpression, context.getVariablesMap()));
        if (right.getValueFrom().equals(ValueFromEnum.refer.name())) {
          String rightExpression =
              VariableUtils.getExpressionFromBracket((String) right.getValue());
          object.put(
              "right",
              getValueFromPayload(rightExpression, context.getVariablesMap()) == null
                  ? ""
      ...
      for (Pair pair : inputs) {
        ...
        if (left.getValue() == null
            || (right.getValue() == null
                && !right.getValueFrom().equals(ValueFromEnum.clear.name()))) {
          // Skip if expression is null, no modification needed
          continue;
        }
        ...
        VariableUtils.setValueForPayload(
            leftExpression, context.getVariablesMap(), valueFromPayload);
      }
    } catch (Exception e) {
      ...
      if (e instanceof BizException) {
        ...
        nodeResult.setError(
            ErrorCode.WORKFLOW_EXECUTE_ERROR.toError(
                "Variable assignment error:" + e.getMessage()));
      }
      return nodeResult;
    }
  ...
  /**
   * Represents a pair of input parameters for variable assignment Left is the target variable,
   * right is the source value
   */
  @Data
