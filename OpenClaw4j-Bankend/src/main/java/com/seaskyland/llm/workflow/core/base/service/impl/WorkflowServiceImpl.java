...
public class WorkflowServiceImpl implements WorkflowService {
  ...
  @Resource private WorkflowExecuteManager workflowExecuteManager;

  @Resource private CacheManager cacheManager;

  @Resource private AppService appService;

  @Resource private CommonConfig commonConfig;
  ...
  public WorkflowResponse call(WorkflowRequest request) {
    ...
    // 筛选出nodeType为End的响应
    List<WorkflowResponse> endNodeResponses =
        allResponses.stream()
  ...
  public Flux<WorkflowResponse> streamCall(Flux<WorkflowRequest> requestFlux) {
    ...
    return requestFlux
        .doOnNext(request -> checkAndInitContext(context, request))
        // request model
  ...
  public TaskRunResponse asyncCall(WorkflowRequest request) {
    ...
    return workflowExecuteManager.runTask(
        appVersion, request.getInputParams(), request.getConversationId(), workflowContext);
  }
  ...
  private void checkAndInitContext(WorkflowContext workflowContext, WorkflowRequest request) {
    ...
    try {
      ...
      String conversationId =
          request.getConversationId() == null ? IdGenerator.uuid() : request.getConversationId();
  ...
  private Flux<WorkflowResponse> streamExecute(
      WorkflowContext workflowContext, WorkflowRequest request) {
                ...
                .forEach(
                    nodeId -> {
                      ...
                      if ((NodeTypeEnum.OUTPUT.getCode().equals(currentNodeResult.getNodeType())
                              ...
                              || NodeTypeEnum.INPUT
                                  .getCode()
                                  .equals(currentNodeResult.getNodeType()))
                          && (NodeStatusEnum.EXECUTING
                                  .getCode()
                              ...
                              || NodeStatusEnum.PAUSE
                                  .getCode()
                                  .equals(currentNodeResult.getNodeStatus()))) {
                        NodeResult copyNodeResult =
                            BeanCopierUtils.copy(currentNodeResult, NodeResult.class);
  ...
  private boolean handleNodeMessage(
      Sinks.Many<WorkflowResponse> sink,
      String requestId,
      String taskId,
      String conversationId,
  ...
  private void handleCompletedMsg(
      Sinks.Many<WorkflowResponse> sink,
  ...
  private void sendFinishMessage(
      Sinks.Many<WorkflowResponse> sink, String requestId, String taskId, String conversationId) {
    WorkflowResponse response = new WorkflowResponse();
    response.setRequestId(requestId);
    response.setTaskId(taskId);
    response.setConversationId(conversationId);
