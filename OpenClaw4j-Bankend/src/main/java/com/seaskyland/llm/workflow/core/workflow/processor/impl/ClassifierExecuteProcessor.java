 ...
 *
 * <p>This class is responsible for handling the execution of classification nodes in the workflow.
 * It provides functionality for: 1. Text classification using LLM models 2. Support for both
 * efficient and advanced classification modes 3. Multi-branch decision making based on
 * classification results 4. Customizable classification conditions and examples 5. Integration with
 * short-term memory for context-aware classification 6. Decision reasoning and thought process
 * tracking 7. Default branch handling for unclassified cases 8. Result deduplication and validation
 *
 * @version 1.0.0-M1
 */
...
public class ClassifierExecuteProcessor extends AbstractExecuteProcessor {
  ...
  private static final Pattern DECISION_PATTERN =
      Pattern.compile("<Decision>：(.*?)\\s|<Decision>：(.*)", Pattern.DOTALL);

  // Pattern for extracting thought process
  private static final Pattern THOUGHT_PATTERN =
      Pattern.compile("(?<=<Thinking>：).*?(?=<Decision>)", Pattern.DOTALL);
  ...
  public NodeResult innerExecute(
    ...
    DecisionAndThoughtAndUsage decisionAndThoughtAndUsage =
        constructDecisionAndThought(node, context, nodeResult);
    TargetIdAndActualDecision targetIdAndActualDecision =
        fetchTargetIdAndActualDecision(graph, node, decisionAndThoughtAndUsage, context);
    targetIdAndActualDecision.setTargetIds(
        removeDuplicates(targetIdAndActualDecision.getTargetIds()));
    targetIdAndActualDecision.setActualDecision(
        removeDuplicates(targetIdAndActualDecision.getActualDecision()));
    targetIdAndActualDecision.setConditionId(
        removeDuplicates(targetIdAndActualDecision.getConditionId()));
    // 构建结束
    // 设置输出变量
   ...
   */
  private TargetIdAndActualDecision fetchTargetIdAndActualDecision(
    ...
    // 默认分支
    Optional<Condition> defaultOptional =
        conditions.stream().filter(condition -> condition.getId().equals("default")).findAny();
      ...
      for (String decision : decisions) {
        if (Integer.parseInt(decision) == -1) {
          ...
          List<String> targetIds =
              graph.outgoingEdgesOf(node.getId()).stream()
          ...
          List<String> targetIds =
              graph.outgoingEdgesOf(node.getId()).stream()
                  .filter(
                      edge -> edge.getSourceHandle().equals(node.getId() + "_" + condition.getId()))
                  .map(Edge::getTarget)
                  .collect(Collectors.toList());
          result.getTargetIds().addAll(targetIds);
   ...
   */
  private DecisionAndThoughtAndUsage constructDecisionAndThought(
    ...
    if ("efficient".equals(config.getModeSwitch())) {
      promptTemplate =
          "You are an intelligent decision-making expert. Your task is to determine which of the candidate categories the given input question/text belongs to.\n"
              + "\n"
    ...
    List<Message> shortTermMemories =
        constructShortTermMemory(node, config.getShortMemory(), context);
    ...
    // 调用模型
    Flux<AgentResponse> stream =
        modelExecuteManager.stream(
            config.getModelConfig().getProvider(),
            config.getModelConfig().getModelId(),
            paramMap,
            messages);
    ...
    stream.subscribe(
        ...
        () -> {
          ...
          log.info(
              "log used for query classify response:{} , requestId:{}",
              content,
          ...
          if ("efficient".equals(config.getModeSwitch())) {
            ...
            if (decisionMatcher.find()) {
              String decisionContent =
                  decisionMatcher.group(1) != null
                      ? decisionMatcher.group(1)
  ...
  public void handleCurrentRound4SelfShortTermMemory(
    ...
    chatMessages.add(
        new ChatMessage(
            MessageRole.ASSISTANT, "Matched classification is " + outputMap.get("subject")));
