...
import static com.seaskyland.llm.workflow.core.base.constants.CacheConstants.APPCODE_CONVERSATION_ID_TEMPLATE;
import static com.seaskyland.llm.workflow.core.base.constants.CacheConstants.WORKFLOW_NODE_SELF_SHORT_MEMORY_TEMPLATE;
...
import static com.seaskyland.llm.workflow.core.workflow.constants.WorkflowConstants.MEMORY_TYPE_CUSTOM;
import static com.seaskyland.llm.workflow.core.workflow.constants.WorkflowConstants.MEMORY_TYPE_SELF;
import static com.seaskyland.llm.workflow.core.workflow.constants.WorkflowConstants.NODE_BRANCH_DEFAULT;
import static com.seaskyland.llm.workflow.core.workflow.constants.WorkflowConstants.NODE_BRANCH_FAIL;
import static com.seaskyland.llm.workflow.core.workflow.constants.WorkflowConstants.NODE_CONFIG_RETRY;
import static com.seaskyland.llm.workflow.core.workflow.constants.WorkflowConstants.NODE_CONFIG_SHORT_MEMORY;
import static com.seaskyland.llm.workflow.core.workflow.constants.WorkflowConstants.NODE_CONFIG_TRY_CATCH;
import static com.seaskyland.llm.workflow.core.workflow.constants.WorkflowConstants.PARAM_TYPE_ARRAY_OBJECT_LOWER_CASE;
import static com.seaskyland.llm.workflow.core.workflow.constants.WorkflowConstants.PARAM_TYPE_BOOLEAN_LOWER_CASE;
import static com.seaskyland.llm.workflow.core.workflow.constants.WorkflowConstants.PARAM_TYPE_NUMBER_LOWER_CASE;
import static com.seaskyland.llm.workflow.core.workflow.constants.WorkflowConstants.PARAM_TYPE_OBJECT_LOWER_CASE;
import static com.seaskyland.llm.workflow.core.workflow.constants.WorkflowConstants.PARAM_TYPE_STRING_LOWER_CASE;
import static com.seaskyland.llm.workflow.core.workflow.constants.WorkflowConstants.SYS_QUERY_KEY;
import static com.seaskyland.llm.workflow.core.workflow.constants.WorkflowConstants.TRY_CATCH_STRATEGY_DEFAULT_VALUE;
import static com.seaskyland.llm.workflow.core.workflow.constants.WorkflowConstants.TRY_CATCH_STRATEGY_FAIL_BRANCH;
import static com.seaskyland.llm.workflow.core.workflow.constants.WorkflowConstants.TRY_CATCH_STRATEGY_NOOP;
...
import com.seaskyland.llm.workflow.runtime.domain.workflow.inner.RetryConfig;
 ...
 *
 * <p>This class provides core functionality for: 1. Node execution flow control (pre-check,
 * execution, retry, exception handling) 2. Variable processing (input/output parameter mapping,
 * template replacement) 3. Short-term memory management (session memory, node memory) 4. Node
 * result handling (status updates, result caching)
 *
 * <p>The processor follows a template method pattern where concrete implementations need to provide
 * specific execution logic through innerExecute().
 */
@Slf4j
@Component
public abstract class AbstractExecuteProcessor implements ExecuteProcessor {
  ...
  public AbstractExecuteProcessor(
  ...
  /**
   * Main execution method for workflow nodes. Handles the complete execution lifecycle including
   * pre-checks, execution, retry, exception handling, variable processing and result handling.
   *
  ...
  /**
   * Pre-execution validation checks. Verifies: 1. Workflow is not stopped 2. Node has valid
   * successors (unless it's an end node)
   *
   * @param graph The workflow graph
   ...
   */
  public abstract NodeResult innerExecute(
      DirectedAcyclicGraph<String, Edge> graph, Node node, WorkflowContext context);

  /**
   * Processes node variables and updates context. Maps input/output parameters and updates variable
   * cache.
   *
   * @param graph The workflow graph
  ...
  /**
   * Handles retry logic for failed node executions. Implements configurable retry with interval and
   * max attempts.
   *
   * @param graph The workflow graph
      ...
      if (nodeResult.getNodeStatus().equals(NodeStatusEnum.FAIL.getCode())) {
        ...
        if (configMap != null && configMap.containsKey(NODE_CONFIG_RETRY)) {
          RetryConfig retryConfig =
              JsonUtils.fromJson(
                  JsonUtils.toJson(configMap.get(NODE_CONFIG_RETRY)), RetryConfig.class);
          if (retryConfig != null && BooleanUtils.isTrue(retryConfig.getRetryEnabled())) {
            // interval default 1000ms
            int retryInterval =
                retryConfig.getRetryInterval() == null ? 1000 : retryConfig.getRetryInterval();
  ...
  /**
   * Handles exception handling logic for node executions. Supports different strategies like
   * default value and fail branch.
   *
   * @param graph The workflow graph
   ...
   */
  public NodeResult handleTryCatch(
      DirectedAcyclicGraph<String, Edge> graph,
      Node node,
      WorkflowContext context,
      NodeResult nodeResult) {
    try {
      ...
      if (configMap != null && configMap.containsKey(NODE_CONFIG_TRY_CATCH)) {
        tryCatchConfig =
            JsonUtils.fromJson(
                JsonUtils.toJson(configMap.get(NODE_CONFIG_TRY_CATCH)), TryCatchConfig.class);
        if (tryCatchConfig != null) {
          strategy =
              tryCatchConfig.getStrategy() == null
                  ? TRY_CATCH_STRATEGY_NOOP
        ...
        if (needTryCatch) {
          if (TRY_CATCH_STRATEGY_DEFAULT_VALUE.equals(strategy)) {
            ...
            nodeResult.setOutput(
                JsonUtils.toJson(
                    construct4DefaultValueStrategy(tryCatchConfig.getDefaultValues())));
            ...
            List<String> failTargetIds =
                edges.stream()
                    .filter(
                        edge -> {
                          if (edge.getSourceHandle()
                              .equals(node.getId() + "_" + NODE_BRANCH_FAIL)) {
                            return true;
                          }
                          return false;
                        })
  ...
  private void handleSelfShortTermMemory(
      Node node, WorkflowContext context, NodeResult nodeResult) {
    if (!nodeResult.getNodeStatus().equals(NodeStatusEnum.SUCCESS.getCode())) {
      return;
    }
    ...
    ShortTermMemory shortMemory =
        JsonUtils.fromJson(
            JsonUtils.toJson(nodeParamMap.get(NODE_CONFIG_SHORT_MEMORY)), ShortTermMemory.class);
  ...
  public void handleCurrentRound4SelfShortTermMemory(
      Node node, Integer round, WorkflowContext context, NodeResult nodeResult) {}

  /**
   * Processes and updates node execution results. Updates node status, execution time, and handles
   * end node completion.
   *
   * @param graph The workflow graph
   * @param node The executed node
  ...
  /**
   * Processes session variables and persists to cache. Variables are stored with 1 hour expiration.
   *
   * @param context The workflow context
   */
  ...
  /**
   * Manages session-level short-term memory. Maintains conversation history for the workflow
   * conversation.
   *
   * @param context The workflow context
  ...
  private void handleSessionShortTermMemory(WorkflowContext context, NodeResult nodeResult) {
    ...
    if (historySwitch) {
      String conversationId =
          String.format(
              APPCODE_CONVERSATION_ID_TEMPLATE, context.getAppId(), context.getConversationId());
      String inputContent =
          context.getSysMap().get(SYS_QUERY_KEY) == null
              ? ""
      ...
      conversationChatMemory.add(
          conversationId, buildCurrentRoundMessages(inputContent, outputContent));
    }
  }

  /**
   * Manages node-level short-term memory. Maintains conversation history for individual nodes.
   *
   * @param context The workflow context
   */
  private void handleNodeSelfShortTermMemory(WorkflowContext context) {
    ...
    nodeResultMap.entrySet().stream()
        .forEach(
            entry -> {
              if (entry == null
                  || entry.getValue() == null
                  || entry.getValue().getShortMemory() == null) {
                return;
              }
              ...
              String cacheKey =
                  String.format(
                      WORKFLOW_NODE_SELF_SHORT_MEMORY_TEMPLATE,
                      context.getAppId(),
                      context.getConversationId(),
                      entry.getValue().getNodeId());
              List<ChatMessage> historyCache = cacheManager.get(cacheKey);
              ...
              cacheManager.put(cacheKey, Lists.newArrayList(historyMessages));
            });
  }
  ...
  public String replaceTemplateContent(String originalTemplate, WorkflowContext context) {
    ...
    for (String key : keys) {
      ...
      if (o == null) {
        ...
        promptContent =
            promptContent.replaceAll(
                "\\$\\{" + key + "}", Matcher.quoteReplacement(replaceContent));
      }
    }
   ...
   */
  public static Map<String, Object> constructOutputParamsMap(
      Node node, Object resultObj, WorkflowContext context) {
    try {
      Map<String, Object> resultMap = Maps.newHashMap();
      ...
      if (resultObj instanceof Map) {
        ...
        constructOutputs(
            (Map<String, Object>) resultObj, finalResultObj, node.getConfig().getOutputParams());
        return finalResultObj;
      } else {
        node.getConfig().getOutputParams().stream()
            .forEach(
                param -> {
      ...
      throw new BizException(
          ErrorCode.WORKFLOW_CONFIG_INVALID.toError(
              "Output format does not match actual structure, original return is:"
   ...
   */
  private static void constructOutputs(
    ...
    for (Node.OutputParam outputParamsRef : outputParamsRefs) {
      ...
      if (typeSet.contains(type)) {
        ...
        if (!CollectionUtils.isEmpty(outputParamsRef.getProperties())) {
          constructArray(
              (List) sourceMap.get(outputParamsRef.getKey()),
              list,
        ...
        if (!CollectionUtils.isEmpty(outputParamsRef.getProperties())) {
          constructOutputs(
              (Map<String, Object>) sourceMap.get(outputParamsRef.getKey()),
              jsonObject,
              outputParamsRef.getProperties());
        } else {
  ...
  private static void constructArray(
      List sourceList, List targetList, List<Node.OutputParam> outputParamsRefs) {
        ...
        if (typeSet.contains(type)) {
          if (o instanceof Map) {
            ...
            if (targetList.size() > i) {
              ((Map) targetList.get(i))
                  .put(outputParamsRef.getKey(), oObj.get(outputParamsRef.getKey()));
            } else {
          ...
          constructOutputs(
              ((Map<String, Object>) ((Map<String, Object>) o).get(outputParamsRef.getKey())),
              targetObj,
              outputParamsRef.getProperties());
          if (targetList.size() > i) {
          ...
          constructArray(
              ((List) ((Map<String, Object>) o).get(outputParamsRef.getKey())),
              jsonArray,
              outputParamsRef.getProperties());
          if (targetList.size() > i) {
          ...
          if (targetList.size() > i) {
            ((Map<String, Object>) targetList.get(i))
                .put(
                    outputParamsRef.getKey(),
                    ((Map<String, Object>) o).get(outputParamsRef.getKey()));
          } else {
            Map<String, Object> newObj = new HashMap<>();
            newObj.put(
                outputParamsRef.getKey(), ((Map<String, Object>) o).get(outputParamsRef.getKey()));
            targetList.add(newObj);
          }
        }
      }
  ...
  protected CheckNodeParamResult checkInputParams(List<Node.InputParam> inputParams) {
    CheckNodeParamResult result = CheckNodeParamResult.success();
    if (!CollectionUtils.isEmpty(inputParams)) {
      List<Node.InputParam> errorInputParams =
          inputParams.stream()
              .filter(
                  inputParam -> {
      ...
      if (!CollectionUtils.isEmpty(errorInputParams)) {
        ...
        result.setErrorInfos(
            errorInputParams.stream()
                .map(
                    inputParam ->
                        "input param [" + inputParam.getKey() + "] is empty or is illegal")
                .collect(Collectors.toList()));
      }
    }
    return result;
   ...
   */
  protected List<Message> constructShortTermMemory(
      Node node, ShortTermMemory shortTermMemory, WorkflowContext context) {
    ...
    String type =
        shortTermMemory.getType() == null ? MEMORY_TYPE_CUSTOM : shortTermMemory.getType();
    if (MEMORY_TYPE_SELF.equals(type)) {
                ...
                if (MessageRole.USER.getValue().equals(role)) {
                  ...
                  throw new BizException(
                      ErrorCode.INVALID_PARAMS.toError(
                          "short term memory", "Short term memory schema is invalid"));
                }
              }
            }
          }
