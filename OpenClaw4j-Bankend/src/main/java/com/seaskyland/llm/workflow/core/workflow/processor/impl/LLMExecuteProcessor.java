 ...
 *
 * <p>This class is responsible for handling the execution of LLM (Large Language Model) nodes in
 * the workflow. It provides functionality for: 1. Processing LLM node execution with streaming
 * support 2. Managing model configurations and parameters 3. Handling system and user prompts 4.
 * Processing vision capabilities for multimodal inputs 5. Managing short-term memory for
 * conversations 6. Error handling and retry mechanisms 7. Support for model parameter customization
 * 8. Integration with file management for media handling 9. Context-aware prompt processing 10.
 * Response streaming and content aggregation
 *
 * @version 1.0.0-M1
 */
...
public class LLMExecuteProcessor extends AbstractExecuteProcessor {
  ...
  public NodeResult innerExecute(
    ...
    log.info(
        context.getRequestId()
            + " LLMExecuteProcessor requestMessages : "
            + JsonUtils.toJson(requestMessages));
    ...
    Flux<AgentResponse> stream =
        modelExecuteManager.stream(
            modelConfig.getProvider(), modelConfig.getModelId(), paramMap, requestMessages);
    ...
    if (agentResponse.isSuccess()) {
      ...
      if (StringUtils.isBlank(responseText)) {
        ...
        nodeResult.setError(
            ErrorCode.WORKFLOW_EXECUTE_ERROR.toError("Model service did not return any response"));
      } else {
  ...
  private void handleResponse(
      ...
      ModelTmpResponseContent tmpResponseContent) {
    if (!response.isSuccess()) {
      ...
      context
          ...
          .put(
              node.getId(),
              NodeResult.error(node, "Workflow execution error: " + error.getMessage()));
    } else {
      ...
      tmpResponseContent
          .getTemporaryReasoningContent()
          .append(reasoningContent == null ? "" : reasoningContent);
      Map<String, Object> tmpMap =
          decorateOutput(tmpResponseContent.getTemporaryContent().toString());
  ...
  public CheckNodeParamResult checkNodeParam(DirectedAcyclicGraph<String, Edge> graph, Node node) {
    ...
    if (StringUtils.isBlank(nodeParam.getPromptContent())
        && StringUtils.isBlank(nodeParam.getSysPromptContent())) {
      result.setSuccess(false);
      result
          .getErrorInfos()
          .add("[User Prompt] and [System Prompt] are not empty at the same time");
    }
    return result;
  }
  ...
  public void handleCurrentRound4SelfShortTermMemory(
    ...
    if (StringUtils.isNotBlank(userPrompt)) {
      chatMessages.add(
          new ChatMessage(MessageRole.USER, replaceTemplateContent(userPrompt, context)));
    }
    Map<String, Object> outputMap = JsonUtils.fromJsonToMap(nodeResult.getOutput());
    chatMessages.add(
        new ChatMessage(MessageRole.ASSISTANT, outputMap.get(OUTPUT_DECORATE_PARAM_KEY)));
