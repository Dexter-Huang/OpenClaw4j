...
import static com.seaskyland.llm.workflow.runtime.enums.ErrorCode.APP_COMPONENT_NOT_FOUND_ERROR;
 ...
 *
 * <p>This processor is responsible for executing application components within workflows,
 * supporting both agent and workflow component types with streaming and non-streaming execution
 * modes.
 *
 * <p>Key Features: 1. Supports multiple component types (Agent and Workflow) 2. Handles streaming
 * and non-streaming execution modes 3. Manages component input/output parameter mapping 4. Supports
 * short-term memory for context-aware execution 5. Provides real-time execution status updates 6.
 * Handles component execution errors and validation 7. Supports JSON and text output formats 8.
 * Manages component execution context and state
 *
 * @author guning.lt
 * @version 1.0.0-M1
 */
...
public class AppComponentExecuteProcessor extends AbstractExecuteProcessor {
  ...
  public AppComponentExecuteProcessor(
  ...
  public NodeResult innerExecute(
    ...
    try {
      ...
      if (nodeParam.getType().equals(AppComponentTypeEnum.Agent.getValue())) {
        ...
        stream
            .doOnNext((modelResponse) -> handleResponse(modelResponse, node, context, builder))
            .blockLast();
        ...
        stream
            .doOnNext(
                (modelResponse) -> handleResponse(modelResponse, node, context, builder, result))
            .blockLast();
   ...
   */
  public HashMap<String, Object> convertToComponentInput(
      List<Node.InputParam> inputParams, WorkflowContext context) {
    HashMap<String, Object> inputJson = new HashMap<>();
    inputParams.forEach(
        inputParam -> {
          if (inputParam.getValueFrom() != null
              && inputParam.getKey() != null
              && inputParam.getValue() != null) {
  ...
  public void handleResponse(
      AgentResponse agentResponse, Node node, WorkflowContext context, StringBuilder builder) {
    NodeParam nodeParam = JsonUtils.fromMap(node.getConfig().getNodeParam(), NodeParam.class);
    ...
    if (!agentResponse.isSuccess()) {
      ...
      context
          .getNodeResultMap()
          .put(node.getId(), NodeResult.error(node, "Node execution failed: " + content));
    } else {
      String responseText;
      Map<String, Object> outputMap = Maps.newHashMap();
  ...
  public void handleResponse(
    ...
    if (!workflowResponse.isSuccess()) {
      ...
      context
          .getNodeResultMap()
          .put(node.getId(), NodeResult.error(node, "Execution failed: " + errorMsg));
    } else {
