...
import static com.seaskyland.llm.workflow.core.base.constants.CacheConstants.CACHE_WORKSPACE_PROVIDER_PREFIX;
...
import com.seaskyland.llm.workflow.core.utils.security.CryptoUtils;
...
public class ProviderManager extends ServiceImpl<ProviderMapper, ProviderEntity> {
    ...
    try {
      ...
      if (existingProvider != null) {
        ...
        throw new BizException(
            ErrorCode.INVALID_PARAMS.toError("input_params", "provider already exists"));
      }
      ...
      providerEntity.setSource(
          StringUtils.isNotBlank(providerConfigInfo.getSource())
              ? providerConfigInfo.getSource()
              : DataSourceEnum.custom.name());
      providerEntity.setEnable(
          providerConfigInfo.getEnable() != null ? providerConfigInfo.getEnable() : true);
      providerEntity.setCredential(
          providerConfigInfo.getCredential() == null
              ? "{}"
      ...
      providerEntity.setProtocol(
          StringUtils.isNotBlank(providerConfigInfo.getProtocol())
              ? providerConfigInfo.getProtocol()
              : "openai");
      ...
      if (isSuccess) {
        ...
        cacheManager.put(key, providerEntity);

        String nonWorkspaceKey = getProviderCacheKey(provider, null);
        cacheManager.put(nonWorkspaceKey, providerEntity);
      }
  ...
  public boolean updateProvider(ProviderConfigInfo providerConfigInfo) {
    ...
    try {
      ...
      if (existingProvider == null) {
        ...
        throw new BizException(
            ErrorCode.INVALID_PARAMS.toError("input_params", "provider not found"));
      }
      ...
      if (isSuccess) {
        String key = getProviderCacheKey(provider, workspaceId);
        cacheManager.put(key, existingProvider);

        String nonWorkspaceKey = getProviderCacheKey(provider, null);
        cacheManager.put(nonWorkspaceKey, existingProvider);
      }
  ...
  public boolean deleteProvider(String provider) {
    ...
    try {
      ...
      if (isSuccess) {
        ...
        String nonWorkspaceKey = getProviderCacheKey(provider, null);
        cacheManager.delete(nonWorkspaceKey);
      }
  ...
  private ProviderEntity getProviderEntity(String provider, String workspaceId) {
    String key = getProviderCacheKey(provider, workspaceId);
    ProviderEntity entity = cacheManager.get(key);
    if (entity != null) {
  ...
  private ProviderConfigInfo toProviderConfig(ProviderEntity entity, boolean mask) {
    ...
    if (StringUtils.isNotBlank(entity.getCredential())) {
      ModelCredential credential =
          JsonUtils.fromJson(entity.getCredential(), ModelCredential.class);
