...
/**
 * A composite tool callback provider that manages and provides tool callbacks from multiple
 * sources: plugins, MCP servers, and application components.
 *
 * @since 1.0.0.3
 */
@Slf4j
...
public class CompositeToolCallbackProvider implements ToolCallbackProvider {
  ...
  public ToolCallback[] getToolCallbacks() {
    ...
    if (!CollectionUtils.isEmpty(agentComponents)) {
      List<ToolCallback> agentComponentCallbacks =
          buildAppComponentCallbacks(agentComponents, AppComponentTypeEnum.Agent);
    ...
    if (!CollectionUtils.isEmpty(workflowComponents)) {
      List<ToolCallback> workflowComponentCallbacks =
   ...
   *
   * <p>This method ensures that each tool has a unique name, which is required for proper tool
  ...
  private void addToolCallbacks(
      List<ToolCallback> toolCallbacks, List<ToolCallback> newToolCallbacks) {
    Set<String> existingNames =
        toolCallbacks.stream()
            .map(callback -> callback.getToolDefinition().name())
            .collect(Collectors.toSet());

    newToolCallbacks.stream()
        .filter(
            toolCallback -> {
  ...
  private List<ToolCallback> buildMcpToolCallbacks(List<AgentConfig.McpServer> mcpServers) {
    ...
    List<McpServerDetail> mcpServerDetails =
        mcpServerService.listByCodes(
            McpQuery.builder().needTools(true).serverCodes(serverCodes).build());
    ...
    for (McpServerDetail mcpServerDetail : mcpServerDetails) {
      if (!CollectionUtils.isEmpty(mcpServerDetail.getTools())) {
        for (McpTool tool : mcpServerDetail.getTools()) {
          toolCallbacks.add(
              new McpToolCallback(mcpServerService, mcpServerDetail, tool, extraParams));
        }
      }
    }
  ...
  private List<ToolCallback> buildAppComponentCallbacks(
      List<String> agentComponents, AppComponentTypeEnum componentType) {
    if (CollectionUtils.isEmpty(agentComponents)) {
      return List.of();
    }

    Map<String, ToolCallSchema> toolCallSchemaMap =
        appComponentManager.getToolCallSchema(agentComponents);
    if (CollectionUtils.isEmpty(toolCallSchemaMap)) {
      return List.of();
    }

    List<ToolCallback> toolCallbacks = new ArrayList<>();
    toolCallSchemaMap.forEach(
        (key, value) -> {
          toolCallbacks.add(
              new AppComponentToolCallback(
                  appComponentManager, key, value, extraParams, componentType));
        });
