...
import static com.seaskyland.llm.workflow.core.base.constants.CacheConstants.CACHE_PROVIDER_LIST_CACHE_PREFIX;
...
import com.seaskyland.llm.workflow.core.utils.security.RSACryptUtils;
import com.seaskyland.llm.workflow.runtime.domain.RequestContext;
import com.seaskyland.llm.workflow.runtime.domain.Result;
import com.seaskyland.llm.workflow.runtime.domain.model.AddModelRequest;
import com.seaskyland.llm.workflow.runtime.domain.model.AddProviderRequest;
import com.seaskyland.llm.workflow.runtime.domain.model.CredentialSpec;
import com.seaskyland.llm.workflow.runtime.domain.model.QueryProviderRequest;
import com.seaskyland.llm.workflow.runtime.domain.model.UpdateModelRequest;
import com.seaskyland.llm.workflow.runtime.domain.model.UpdateProviderRequest;
...
/**
 * Controller for managing model providers and their configurations in the SAA Studio platform.
 *
 * <p>This controller provides comprehensive REST endpoints for:
 *
 * <ul>
 *   <li>Provider lifecycle management (CRUD operations)
 *   <li>Model management within providers
 *   <li>Credential and authentication handling
 *   <li>Protocol and configuration management
 *   <li>Parameter rule management for models
 * </ul>
 *
 * Key features: - Support for multiple provider protocols (OpenAI, custom, etc.) - Secure
 * credential management with RSA encryption - Model type and parameter validation - Caching and
 * performance optimization - Comprehensive error handling
 *
 * @since 1.0.0.3
 */
@Slf4j
...
public class ProviderController {
   ...
   *
   * <p>This endpoint creates a new provider with the following features: - Generates a unique
   * provider code - Configures provider metadata (name, description, icon) - Sets up supported
   * model types - Handles credential encryption for sensitive data - Manages protocol-specific
   * configurations
   *
  ...
  public Result<Boolean> addProvider(@RequestBody AddProviderRequest request) {
    ...
    if (StringUtils.isNotBlank(request.getSupportedModelTypes())) {
      supportedModelTypes.addAll(
          Arrays.stream(request.getSupportedModelTypes().split(",")).collect(Collectors.toList()));
    } else {
      // Set default to llm
   ...
   *
   * <p>This endpoint modifies provider configurations including: - Basic provider information -
   * Supported model types - Protocol settings - Credential configurations - Provider status
   * (enabled/disabled)
   *
  ...
  public Result<Boolean> updateProvider(
      @PathVariable("provider") String provider, @RequestBody UpdateProviderRequest request) {
    ...
    if (request.getCredentialConfig() != null) {
      ...
      if (StringUtils.isBlank(request.getProtocol())
        ...
        if (CollectionUtils.isEmpty(credentialSpecs)) {
          ...
          credentialSpecs.stream()
              .forEach(
                  credentialSpec -> {
                    if (credentialSpec.isSensitive()) {
                      tmpCredentialConfig.put(
                          credentialSpec.getCode(),
                          RSACryptUtils.encrypt(
   ...
   *
   * <p>This endpoint removes a provider and its associated configurations: - Removes provider
   * metadata - Cleans up associated models - Updates cache
   *
  ...
  public Result<Boolean> deleteProvider(@PathVariable("provider") String provider) {
    if (StringUtils.isBlank(provider)) {
      throw new BizException(
          ErrorCode.INVALID_PARAMS.toError("input_params", "providerId is required"));
    }
   ...
   *
   * <p>This endpoint retrieves a list of providers based on query parameters: - Supports filtering
   * by various criteria - Returns paginated results - Includes provider metadata and status
   *
   * @param request The query parameters for filtering providers
   * @return Result containing a list of matching providers
   */
  @GetMapping
  public Result<List<ProviderConfigInfo>> queryProviders(
      @ModelAttribute QueryProviderRequest request) {
    RequestContext requestContext = RequestContextHolder.getRequestContext();
    if (request == null) {
      throw new BizException(ErrorCode.INVALID_PARAMS.toError("input_params", "request is valid"));
    }
    List<ProviderConfigInfo> providerConfigInfos =
        cacheManager.get(CACHE_PROVIDER_LIST_CACHE_PREFIX + requestContext.getWorkspaceId());
    ...
    cacheManager.put(
        CACHE_PROVIDER_LIST_CACHE_PREFIX + requestContext.getWorkspaceId(),
        providerConfigInfos,
        Duration.ofHours(12));
    return Result.success(providerConfigInfos);
  }
   ...
   *
   * <p>This endpoint returns comprehensive provider information including: - Provider metadata -
   * Configuration details - Supported model types - Protocol information
   *
   ...
   *
   * <p>This endpoint creates a new model configuration: - Validates model parameters - Sets up
   * model metadata - Configures model-specific settings
   *
  ...
  public Result<Boolean> addModel(
      @PathVariable("provider") String provider, @RequestBody AddModelRequest request) {
    ...
    String type =
        request.getType() == null ? ModelConfigInfo.ModelTypeEnum.llm.name() : request.getType();
   ...
   *
   * <p>This endpoint modifies model settings including: - Model metadata (name, tags, icon) - Model
   * status (enabled/disabled) - Model-specific configurations
   *
   * @param provider The provider code
   ...
   *
   * <p>This endpoint removes a model and its associated configurations: - Removes model metadata -
  ...
  public Result<Boolean> deleteModel(
      @PathVariable("provider") String provider, @PathVariable("modelId") String modelId) {
    RequestContext requestContext = RequestContextHolder.getRequestContext();
    if (StringUtils.isBlank(modelId)) {
      throw new BizException(
          ErrorCode.INVALID_PARAMS.toError("input_params", "modelId is required"));
    }
    cacheManager.delete(CACHE_PROVIDER_LIST_CACHE_PREFIX + requestContext.getWorkspaceId());
   ...
   *
   * <p>This endpoint returns all models associated with a provider: - Includes model metadata -
   * Includes model status - Includes model configurations
   *
   * @param provider The provider code
   ...
   *
   * <p>This endpoint returns comprehensive model information including: - Model metadata -
  ...
  public Result<ModelConfigInfo> getModelDetail(
      @PathVariable("provider") String provider, @PathVariable("modelId") String modelId) {
    if (StringUtils.isBlank(modelId)) {
      throw new BizException(
          ErrorCode.INVALID_PARAMS.toError("input_params", "modelId is required"));
   ...
   *
   * <p>This endpoint returns the parameter configuration rules for a model: - Parameter validation
   * rules - Required and optional parameters - Parameter types and constraints
   *
   * @param provider The provider code
   * @param modelId The model identifier
  ...
  public Result<List<ParameterRule>> getModelParamRules(
      @PathVariable("provider") String provider, @PathVariable("modelId") String modelId) {
    if (StringUtils.isBlank(provider) || StringUtils.isBlank(modelId)) {
      throw new BizException(
          ErrorCode.INVALID_PARAMS.toError("input_params", "provider or modelId is required"));
    }
   ...
   *
   * <p>This endpoint returns all supported protocol types: - Protocol names - Protocol versions (if
   * applicable)
   *
