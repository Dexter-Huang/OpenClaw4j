...
import com.seaskyland.llm.workflow.core.base.manager.SandboxManager;
 ...
 *
 * <p>This processor is responsible for executing script nodes in the workflow, supporting both
 * Python and JavaScript scripts. It provides a secure execution environment through sandbox
 * isolation and handles script input/output parameter management.
 *
 * <p>Key Features: 1. Supports Python and JavaScript script execution 2. Provides sandbox isolation
 * for secure script execution 3. Manages script input/output parameter mapping 4. Handles script
 * execution errors and validation 5. Supports variable injection from workflow context 6. Validates
 * script content and parameters 7. Processes script execution results 8. Manages script execution
 * environment
 *
...
public class ScriptExecuteProcessor extends AbstractExecuteProcessor {
  ...
  public NodeResult innerExecute(
    ...
    if (!Arrays.stream(ScriptType.values()).toList().contains(ScriptType.fromString(scriptType))) {
      ...
      nodeResult.setErrorInfo(
          "Unsupported script type: "
              + scriptType
              + ", currently only python3 and javascript are supported");
      return nodeResult;
    }
    ...
    if (executeResult != null && executeResult.isSuccess()) {
      ScriptExecutionResponse scriptRes =
          JsonUtils.fromJson(executeResult.getData(), ScriptExecutionResponse.class);
      if (scriptRes.getSuccess()) {
        ...
        if (scriptRes.getData() instanceof Map) {
          ...
          nodeResult.setErrorCode(ErrorCode.WORKFLOW_CONFIG_INVALID.getCode());
          nodeResult.setErrorInfo(
              "Output format does not match configuration, raw data: " + scriptRes.getData());
        }
      } else {
          ...
          if (valueFrom.equals(ValueFromEnum.refer.name())) {
            ...
            Object value =
                VariableUtils.convertValueByType(
                    scriptParam.getKey(), scriptParam.getType(), scriptParam.getValue());
            if (value == null) {
              return;
            }
            map.put(scriptParam.getKey(), value);
