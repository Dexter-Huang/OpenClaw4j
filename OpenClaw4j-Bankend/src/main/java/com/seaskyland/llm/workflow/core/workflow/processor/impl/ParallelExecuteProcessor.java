...
import static com.seaskyland.llm.workflow.core.utils.common.VariableUtils.getValueFromPayload;
 ...
 *
 * <p>This processor is responsible for executing multiple workflow tasks in parallel, supporting
 * concurrent execution of sub-workflows with configurable batch size and concurrency limits.
 *
 * <p>Key Features: 1. Parallel execution of sub-workflows with configurable concurrency 2. Batch
 * processing with size limits and monitoring 3. Dynamic task scheduling and resource management 4.
 * Real-time progress monitoring and status tracking 5. Error handling and task failure management
 * 6. Context isolation for parallel tasks 7. Result aggregation and output formatting 8.
 * Redis-based state management for distributed execution
 *
 * @author guning.lt
 * @version 1.0.0-M1
 */
...
public class ParallelExecuteProcessor extends AbstractExecuteProcessor {
   ...
   * @param cacheManager Redis manager for state management
   ...
   */
  public ParallelExecuteProcessor(
  ...
  public NodeResult innerExecute(
    ...
    log.info(
        ...
        nodeIds,
        JsonUtils.toJson(appOrchestraConfig),
        maxIndex);
    ...
    if (workflowInnerService.checkRedisNecessity(context)) {
      ...
      ThreadPoolUtils.taskExecutorService.submit(
          () -> {
            try {
              executeMonitorThread(
                  context, contextList, nodeIds, node, subTaskStatus, finalMaxIndex, nodeSetList);
            } catch (Exception e) {
              log.error(
                  "ParallelProcessor task Schedule execute,requestId:{} error:{}",
                  context.getRequestId(),
                  e.getMessage());
            }
          });
          ...
          ThreadPoolUtils.taskExecutorService.submit(
              () -> {
                ...
                try {
                  ...
                  log.error(
                      "Task error batchIndex:{},RequestID:{}:",
                      finalIndex,
                      context.getRequestId(),
                      e);
                } finally {
    ...
    while (true) {
      ...
      if (!subTaskStatus.get()) {
        ...
        log.info(
            "ParallelExecuteProcessor is fail requestId:{} ,nodeResultFail:{} ",
            context.getRequestId(),
  ...
  private void executeMonitorThread(
      ...
      ArrayList<WorkflowContext> contextList,
      List<String> nodeIds,
      Node node,
      AtomicBoolean subTaskStatus,
      int maxIndex,
   ...
   * @param nodeSetList List of successful node sets
   */
  private void extraSubTaskResult(
      WorkflowContext context,
      ArrayList<WorkflowContext> contextList,
      List<String> nodeIds,
      Node node,
