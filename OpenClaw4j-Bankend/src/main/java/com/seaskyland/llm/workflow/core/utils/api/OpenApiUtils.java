...
/**
 * Utility class for handling OpenAPI operations. Provides methods for parsing, converting and
 * manipulating OpenAPI specifications.
 *
 * @since 1.0.0.3
 */
public class OpenApiUtils {
  ...
  private static final List<String> NO_BODY_PARAMS_LOCATION =
      Arrays.asList("Path", "Query", "Header");
  ...
  @Getter private static final String DEFINED_EXTENSION = "x-source";

  /** Token type identifier */
  @Getter private static final String TOKEN_TYPE = "token";

  /** User source extension value */
  @Getter private static final String EXTENSION_USER_SOURCE = "user";
   ...
   */
  private static void paramToYamlParam(
      List<Parameter> parameters, List<ApiParameter> inputYamlParamList) {
    if (parameters == null) {
      return;
    }
  ...
  public static Map<String, Tool> parseSchemaToForm(String yamlString, boolean formatCheck) {
    ...
    SwaggerParseResult parseResult =
        new OpenAPIV3Parser().readContents(yamlString, null, parseOptions);
                          ...
                          properties.forEach(
                              (name, nameSchema) -> {
                                ...
                                if (nameSchema.getExtensions() != null) {
                                  Object paramSource =
                                      nameSchema.getExtensions().get(DEFINED_EXTENSION);
                                  if (paramSource instanceof String
                                      && String.valueOf(paramSource)
                                          .equals(EXTENSION_USER_SOURCE)) {
                                  ...
                                  if ("object".equals(itemType)) {
                                    ...
                                    if (schemItem.getProperties() != null) {
                                      processSubProperties(
                                          schemItem.getProperties(),
                                          schemItem.getRequired(),
                                          yamlParamInfo);
                                    }
                                  } else {
                                    yamlParamInfo.setType(
                                        "Array<" + firstLetterToUpperCase(itemType) + ">");
                                  }
                                } else if ("object".equals(nameSchemaType)) {
                                  yamlParamInfo.setType("Object");
                                  Map<String, Schema> nameSchemaProperties =
                                      nameSchema.getProperties();
                                  if (nameSchemaProperties != null
                                      && !nameSchemaProperties.isEmpty()) {
                                    processSubProperties(
                                        nameSchemaProperties,
                                        nameSchema.getRequired(),
                                        yamlParamInfo);
                                  }
                                } else {
                        ...
                        (name, nameSchema) -> {
                          ...
                          if ("array".equals(nameSchemaType)) {
                            ...
                            if (nameSchemaProperties != null && !nameSchemaProperties.isEmpty()) {
                              processSubProperties(
                                  nameSchemaProperties, nameSchema.getRequired(), yamlParamInfo);
                            }
                          } else {
                            yamlParamInfo.setType(firstLetterToUpperCase(nameSchemaType));
                          }
   ...
   */
  private static void buildToolFunctionParameters(
   ...
   */
  private static void processSubProperties(
      Map<String, Schema> properties, List<String> requiredKeys, ApiParameter yamlParamInfo) {
    properties.forEach(
        (key, value) -> {
          ...
          .forEach(
              (contentType, mediaType) -> {
                ...
                if (mediaTypeSchema != null
                    && !CollectionUtils.isEmpty(mediaTypeSchema.getProperties())) {
                  mediaTypeSchema
                      .getProperties()
                      .forEach(
                          (propName, propSchema) ->
                              checkSchemaExtension(propName, (Schema<?>) propSchema, result, 0));
                }
              });
    }
    return result;
   ...
   */
  private static void checkSchemaExtension(
      String schemaField, Schema<?> schema, List<ApiParameter> resultList, int currentDepth)
      throws YAMLException {
    if (currentDepth >= MAX_DEPTH) {
      throw new YAMLException(
          String.format("Schema extensions recursion depth limit(%d) reached.", MAX_DEPTH));
    }
          ...
          .forEach(
              (propName, propSchema) ->
                  checkSchemaExtension(
                      propName, (Schema<?>) propSchema, resultList, currentDepth + 1));
    }
  }
              ...
              pathItem
                  ...
                  .forEach(
                      operation -> {
                        ...
                        if (operation != null) {
                          List<ApiParameter> subResult =
                              parseOpenAPIRestfulMethodWithOutUserSource(operation);
  ...
  public static String buildOpenAPIYaml(Plugin plugin, Tool tool) {
    ...
    // set header info
    Info info =
        new Info().description(plugin.getDescription()).title(plugin.getName()).version("v1");
    openAPI.info(info);
      ...
      inputParams.forEach(
          item -> {
            ...
            if (StringUtils.isNotBlank(item.getDefaultValue())
                && !description.contains(DEFAULT_VALUE_WORD)) {
              item.setDescription(
                  description + DEFAULT_VALUE_WORD + "，请使用" + item.getDefaultValue());
            }
   ...
   * @return List of API parameters
   */
  private static List<ApiParameter> parseOpenAPIRestfulMethodWithOutUserSource(
      Operation operation) {
    List<ApiParameter> result = new ArrayList<>();
    ...
    if (operation.getParameters() != null) {
      operation
          .getParameters()
          .forEach(parameter -> checkParameterExtensionWithOutUserSource(parameter, result));
    }
      ...
      operation
          ...
          .forEach(
              (contentType, mediaType) -> {
                Schema<?> mediaTypeSchema = mediaType.getSchema();
                if (mediaTypeSchema != null
                    && mediaTypeSchema.getProperties() != null
                  ...
                  mediaTypeSchema
                      ...
                      .forEach(
                          (propName, propSchema) -> {
                            checkSchemaExtensionWithOutUserSource(
                                propName, (Schema<?>) propSchema, result, 0, null, requiredKeys);
                          });
                }
              });
    }
   ...
   * @throws YAMLException If recursion depth limit is reached
   */
  private static void checkSchemaExtensionWithOutUserSource(
      String schemaField,
      Schema<?> schema,
      List<ApiParameter> resultList,
      int currentDepth,
      String parentName,
          ...
          .forEach(
              (propName, propSchema) ->
                  checkSchemaExtensionWithOutUserSource(
                      ...
                      parentName == null
                          ? schemaField
                          : parentName + TOOL_FIELDS_CONNECTOR + schemaField,
                      null));
    } else {
      ApiParameter param = new ApiParameter();
      param.setKey(
          parentName == null ? schemaField : parentName + TOOL_FIELDS_CONNECTOR + schemaField);
      param.setType(type);
   ...
   * @param resultList List to store results
   */
  private static void checkParameterExtensionWithOutUserSource(
      Parameter parameter, List<ApiParameter> resultList) {
    Map<String, Object> extensions = parameter.getExtensions();
    boolean needAdd = true;
   ...
   * @param requiredList List of required parameters
   */
  private static void exchangeSchema(
      List<ApiParameter> allParams, Map<String, Schema> propertiesMap, List<String> requiredList) {
