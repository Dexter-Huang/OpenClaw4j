 ...
 *
 * <p>This processor is responsible for handling the output node in the workflow. It processes text
 * templates with variable substitution and supports streaming output.
 *
 * <p>Features: 1. Variable substitution in output templates 2. Support for streaming output 3.
 * Integration with system and conversation variables 4. Dynamic content generation based on node
 * dependencies
 *
 * @version 1.0.0
 */
...
public class OutputExecuteProcessor extends AbstractExecuteProcessor {
   ...
   */
  public void handleTextTemplate(
            ...
            if (nodeContain) {
              ...
              if (StringUtils.isNotBlank(keyFromNodeResult.getOutput())) {
                ...
                if (keyFromNodeResult.getNodeType().equals(NodeTypeEnum.START.getCode())) {
                  ...
                  Map<String, Object> keyFromMap =
                      JsonUtils.fromJsonToMap(context.getNodeResultMap().get(keyFrom).getOutput());
                  // Extract subpath from complete expression, e.g.,
                  // "xxx.yyy" from "LLM_sss.xxx.yyy"
                  contentStringBuilder.append(
                      VariableUtils.getValueStringFromPayload(subPath, keyFromMap));
                }
              }
              if (streamSwitch) {
                nodeResult.setOutput(contentStringBuilder.toString());
  ...
  private boolean isFinished(
      DirectedAcyclicGraph<String, Edge> graph, WorkflowContext context, String nodeId) {
    ...
    if (context.getTaskStatus().equals(NodeStatusEnum.FAIL.getCode())
      ...
      throw new BizException(
          ErrorCode.WORKFLOW_EXECUTE_ERROR.toError(context.getError().getMessage()));
    }
