...
public class AdminCompatService {
  ...
  public AdminPage<Map<String, Object>> list(
      String table, String orderColumn, long pageNumber, long pageSize) {
    ...
    return AdminPage.of(
        ...
        safePageNumber,
        safePageSize,
        rows.stream().map(this::camelize).toList());
  }

  public Map<String, Object> getById(String table, Long id) {
    List<Map<String, Object>> rows =
        jdbcTemplate.queryForList("SELECT * FROM " + table + " WHERE id = ?", id);
    return rows.isEmpty() ? null : camelize(rows.get(0));
  }

  public Map<String, Object> getPrompt(String promptKey) {
    List<Map<String, Object>> rows =
        jdbcTemplate.queryForList("SELECT * FROM prompt WHERE prompt_key = ?", promptKey);
    return rows.isEmpty() ? null : camelize(rows.get(0));
  }
  ...
  public Map<String, Object> getPromptVersion(String promptKey, String version) {
    List<Map<String, Object>> rows =
        jdbcTemplate.queryForList(
            "SELECT * FROM prompt_version WHERE prompt_key = ? AND version = ?",
            promptKey,
            version);
    return rows.isEmpty() ? null : camelize(rows.get(0));
  }
    ...
    int updated =
        jdbcTemplate.update(
            """
				...
				""",
            string(body, "versionDescription"),
            string(body, "template"),
            string(body, "variables"),
            string(body, "modelConfig"),
            string(body, "status"),
            promptKey,
            version);
    if (updated == 0) {
  ...
  public Map<String, Object> createEvaluatorVersion(Map<String, Object> body) {
    ...
    String version =
        StringUtils.hasText(string(body, "version"))
            ? string(body, "version")
  ...
  public Map<String, Object> createExperiment(Map<String, Object> body) {
    Long id =
        insertAndReturnId(
            ...
            string(body, "name"),
            string(body, "description"),
            longValue(body, "datasetId"),
            longValue(body, "datasetVersionId"),
            string(body, "datasetVersion"),
            string(body, "evaluationObjectConfig"),
            string(body, "evaluatorConfig"));
  ...
  public Map<String, Object> updateNamed(String table, Long id, Map<String, Object> body) {
    jdbcTemplate.update(
        "UPDATE "
            + table
            + " SET name = ?, description = ?, update_time = CURRENT_TIMESTAMP WHERE id = ?",
        string(body, "name"),
        string(body, "description"),
        id);
    return getById(table, id);
  ...
  private void copyAlias(Map<String, Object> data, String from, String to) {
    if (data.containsKey(from)
        && (!data.containsKey(to) || data.get(to) == null || "".equals(data.get(to)))) {
  ...
  private String nextVersion(String table, String ownerColumn, Long ownerId) {
    Long count =
        jdbcTemplate.queryForObject(
            "SELECT COUNT(1) FROM " + table + " WHERE " + ownerColumn + " = ?",
            Long.class,
            ownerId);
