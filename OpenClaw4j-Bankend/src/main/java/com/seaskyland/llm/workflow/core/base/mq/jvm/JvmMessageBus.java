 ...
 *
 * <p>This bus is the shared infrastructure used by {@link JvmMqProducer} and
 * {@link JvmMqConsumer}. It provides:
 * <ul>
 *   <li><b>Synchronous publish</b> – delivered inline on the calling thread.</li>
 *   <li><b>Asynchronous publish</b> – dispatched to a cached thread pool.</li>
 *   <li><b>Delayed publish</b> – scheduled via a single-thread scheduler.</li>
 * </ul>
 *
 * <p><strong>Note:</strong> This implementation is single-node only. Messages are
 * never persisted and are lost on JVM restart or application crash.
 *
 * @since 1.0.0.3
 */
@Slf4j
public class JvmMessageBus {
	...
	/**
	 * topic → all registered handlers for that topic.
	 * Uses {@link CopyOnWriteArrayList} so that iteration during dispatch is safe
	 * even if handlers are added concurrently.
	 */
	private final ConcurrentHashMap<String, CopyOnWriteArrayList<MqConsumerHandler<MqMessage>>> subscribers =
	...
	/**
	 * Registers {@code handler} to receive messages published on {@code topic}.
	...
	/**
	 * Publishes {@code message} synchronously. All handlers registered for the
	 * message's topic are invoked on the calling thread before this method returns.
	 *
	...
	/**
	 * Publishes {@code message} asynchronously. The caller is not blocked; delivery
	 * happens on the internal thread pool. {@code callback} is notified after all
	 * handlers have been called (or if an error occurs).
	 */
	public void publishAsync(MqMessage message, SendCallback callback) {
		asyncExecutor.submit(() -> {
			try {
				...
				log.error("[JvmMQ] async publish error on topic '{}': {}", message.getTopic(), e.getMessage(), e);
				if (callback != null) {
					callback.onError(e);
				}
			}
	...
	public void publishDelay(MqMessage message, int delaySeconds) {
		...
		log.debug("[JvmMQ] delayed message queued for topic '{}', delay={}s",
				message.getTopic(), delaySeconds);
	}
