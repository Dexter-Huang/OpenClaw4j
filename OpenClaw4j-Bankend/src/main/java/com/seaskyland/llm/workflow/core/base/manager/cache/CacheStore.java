...
/**
 * Abstraction for cache storage backends. Implementations can be based on Redis (Redisson)
 * or JVM in-memory storage, selected via {@code cache.type} in {@code application.yml}.
 *
 * <p>All key parameters are received <em>as-is</em> (including any prefix already applied
 * by the caller). Implementations must NOT add extra prefix internally.
 *
 * @since 1.0.0.3
 */
public interface CacheStore {
	...
	/**
	 * Adds an element only when the current set size is below {@code fixedSize}.
	 * Returns {@code true} if the element was added.
	 */
	...
	/**
	 * Acquires a lock with blocking-wait semantics. Returns {@code true} if acquired
	 * within {@code waitTimeout}.
	 */
