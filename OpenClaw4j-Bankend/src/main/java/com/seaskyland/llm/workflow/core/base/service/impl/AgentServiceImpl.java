...
import com.seaskyland.llm.workflow.core.agent.AgentExecutor;
...
/**
 * Implementation of agent service for handling agent inference requests. Supports both basic and
 * workflow agent types with synchronous and streaming responses.
 *
 * @since 1.0.0.3
 */
@Service
public class AgentServiceImpl implements AgentService {
  ...
  private Flux<AgentResponse> streamExecute(AgentContext context, AgentRequest request) {
    if (context.getAppType() == AppType.BASIC) {
      ...
      throw new BizException(
          ErrorCode.APP_TYPE_NOT_SUPPORT.toError(context.getAppType().getValue()));
    }
  }
  ...
  private Mono<AgentResponse> handleThrowable(AgentContext context, Throwable err) {
    ...
    if (err instanceof BizException be) {
      ...
      error =
          ErrorHandlerUtils.parseOpenAiError(
              wre.getStatusCode().value(), wre.getResponseBodyAsString());
    } else {
  ...
  private void statistics(AgentContext context, SignalType signalType) {
    ...
    if (SignalType.CANCEL == signalType) {
      ...
      LogUtils.monitor(
          context, "AgentService", "handleRequests", context.getStartTime(), FAIL, null, response);
    }
