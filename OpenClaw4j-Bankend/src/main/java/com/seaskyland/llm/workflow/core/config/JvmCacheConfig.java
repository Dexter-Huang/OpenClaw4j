 ...
 *
 * <p>Activated by setting {@code cache.type=JVM} in {@code application.yml}.
 * No Redis connection is required in this mode.
 *
 * <p><strong>Limitations in JVM mode:</strong>
 * <ul>
 *   <li>Cache is local to the JVM – not shared across multiple instances.</li>
 *   <li>{@link CacheManager#getSet(String)}
 *       returns {@code null}; use the typed set helper methods instead.</li>
 *   <li>Distributed locking ({@code lockWithRedisson}) uses {@link java.util.concurrent.locks.ReentrantLock}
 *       and is therefore <em>not</em> distributed.</li>
 * </ul>
 *
