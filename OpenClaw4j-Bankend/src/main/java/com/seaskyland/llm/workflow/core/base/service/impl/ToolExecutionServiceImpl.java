...
import com.seaskyland.llm.workflow.runtime.enums.ApiParameterLocation;
...
/**
 * Implementation of the tool execution service. Handles the execution of tools and API calls based
 * on tool configurations.
 *
 * @since 1.0.0.3
 */
@Service
public class ToolExecutionServiceImpl implements ToolExecutionService {
  ...
  public ToolExecutionServiceImpl(
      PluginService pluginService, HttpClientManager httpClientManager) {
    this.pluginService = pluginService;
    this.httpClientManager = httpClientManager;
  }

  /**
   * Executes a tool based on the provided request. Validates the request and retrieves necessary
   * tool and plugin information.
   *
  ...
  /**
   * Makes an API call based on the tool configuration. Handles authentication, parameter mapping,
  ...
  public ToolExecutionResult callOpenApi(ToolExecutionRequest request) {
    try {
      ...
      StringBuilder requestUrl =
          new StringBuilder(String.format("%s%s", server, toolConfig.getPath()));
      ...
      if (pluginConfig.getAuth() != null
          && pluginConfig.getAuth().getType() != Plugin.ApiAuthType.NONE) {
        ...
        if (authType == Plugin.AuthorizationType.BEARER) {
          ...
          if (auth.getAuthorizationPosition() == Plugin.AuthorizationPosition.HEADER) {
            ...
            throw new BizException(
                ErrorCode.TOOL_PARAMS_INVALID.toError(
                    "authorization_position", "position must be header or query"));
          }
        } else {
          throw new BizException(
              ErrorCode.TOOL_PARAMS_INVALID.toError(
                  "authorization_type", "type must be basic, bearer or custom"));
        }
      }
      ...
      for (ApiParameter apiParameter : apiParameters) {
        ApiParameterLocation location =
            ApiParameterLocation.of(StringUtils.lowerCase(apiParameter.getLocation()));
        if (location == null) {
          continue;
        }
      ...
      if (method.equalsIgnoreCase(HttpClientManager.Method.GET.getValue())) {
        ...
        result =
            httpClientManager.doGetWithRequestId(
                request.getRequestId(), requestUrl.toString(), headers, parameters);
      } else if (method.equalsIgnoreCase(HttpClientManager.Method.POST.getValue())) {
        String encodedParams =
            HttpClientManager.encodingParams(queryParameters, StandardCharsets.UTF_8.toString());
        ...
        if (MediaType.APPLICATION_FORM_URLENCODED_VALUE.equalsIgnoreCase(contentType)) {
          result =
              httpClientManager.doPostFormWithRequestId(
                  request.getRequestId(), requestUrl.toString(), headers, parameters);
        } else if (MediaType.APPLICATION_JSON_VALUE.equalsIgnoreCase(contentType)) {
          result =
              httpClientManager.doPostJsonWithRequestId(
                  request.getRequestId(), requestUrl.toString(), headers, parameters);
        } else {
          throw new BizException(
              ErrorCode.TOOL_PARAMS_INVALID.toError(
                  "contentType", "contentType " + contentType + " not supported"));
        }
      } else {
        throw new BizException(
            ErrorCode.TOOL_PARAMS_INVALID.toError("method", "method must be get or post"));
      }

      if (!result.isSuccess()) {
        LogUtils.error(
            "call openapi failed, request: {}, result: {}",
  ...
  /**
   * Validates the input parameters against the tool's configuration. Checks for required parameters
   * and their types.
   *
  ...
  public void validateInputs(ToolExecutionRequest request) {
    ...
    if (!CollectionUtils.isEmpty(inputParams)) {
      Map<String, ApiParameter> parameterMap =
          inputParams.stream()
      ...
      for (Map.Entry<String, ApiParameter> entry : parameterMap.entrySet()) {
        ...
        if (paramValue != null && !apiParamType.isValidType(paramValue)) {
          throw new BizException(
              ErrorCode.TOOL_PARAMS_INVALID.toError(
                  entry.getKey(), "param type and value not match"));
        }
      }
    }
  }
  ...
  public Map<String, Object> buildOutputs(
      Map<String, Object> respJSONObj, List<ApiParameter> outputParams) {
  ...
  /**
   * Recursively constructs output objects based on parameter types. Handles different data types
        ...
        case OBJECT:
          {
            ...
            constructOutputs(
                (Map<String, Object>) sourceObject.get(key), jsonObject, apiParam.getProperties());
            targetObj.put(key, jsonObject);
            break;
          }
