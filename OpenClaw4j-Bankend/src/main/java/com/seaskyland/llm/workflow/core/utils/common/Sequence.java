...
/**
 * A distributed unique ID generator based on Twitter's Snowflake algorithm. Generates 64-bit unique
 * IDs using timestamp, datacenter ID, worker ID and sequence number.
 *
 * @since 1.0.0.3
 */
public class Sequence {
  ...
  public Sequence(InetAddress inetAddress) {
    ...
    if (end - start > Sequence.MAX_START_INTERVAL_TIME) {
      ...
      logger.warn(
          "Initialization Sequence Very Slow! Get datacenterId:{} workerId:{}",
          this.datacenterId,
          this.workerId);
    } else {
      initLog();
  ...
  private void initLog() {
    if (logger.isDebugEnabled()) {
      logger.debug(
          "Initialization Sequence datacenterId:{} workerId:{}", this.datacenterId, this.workerId);
    }
  }
