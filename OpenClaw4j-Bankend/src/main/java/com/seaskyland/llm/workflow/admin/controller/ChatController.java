...
import com.seaskyland.llm.workflow.runtime.domain.workflow.debug.*;
...
/**
 * Controller for handling chat and workflow completions. Provides endpoints for both streaming and
...
public class ChatController {
  ...
  public ChatController(
      AgentService agentService, WorkflowService workflowService, CacheManager cacheManager) {
  ...
  /**
   * Handles chat completion requests. Supports both streaming and non-streaming responses.
   *
  ...
  /**
   * Handles workflow completion requests. Supports both streaming and non-streaming responses.
   *
   ...
   */
  private Mono<WorkflowResponse> handleError(
      RequestContext context, WorkflowRequest request, Throwable err) {
    LogUtils.monitor(
        context,
        "ChatController",
        "endStreamCallError",
  ...
  private void handleComplete(RequestContext context, SignalType signalType, SseEmitter emitter) {
    emitter.complete();
    LogUtils.monitor(
        context, "ChatController", "endStreamCall", context.getStartTime(), SUCCESS, null, null);
  }
  ...
  public Result<AsyncResultResponse> getAsyncResults(@RequestBody AsyncResultRequest request) {
    ...
    try {
      ...
      WorkflowContext wfContext = cacheManager.get(cacheKey);

      if (wfContext == null) {
        log.info(
            "Async task not found: taskId={}, workspaceId={}, requestId={}",
            request.getTaskId(),
            context.getWorkspaceId(),
            context.getRequestId());
        return Result.error(
            context.getRequestId(), ErrorCode.WORKFLOW_CONFIG_INVALID.toError("taskId not exists"));
      }
