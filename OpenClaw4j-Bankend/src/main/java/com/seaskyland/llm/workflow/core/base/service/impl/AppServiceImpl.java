...
import static com.seaskyland.llm.workflow.core.base.constants.CacheConstants.*;
...
import com.seaskyland.llm.workflow.runtime.domain.app.AppQuery;
...
/**
 * Implementation of the application service. Handles CRUD operations and version management for
 * applications.
 *
 * @since 1.0.0.3
 */
...
public class AppServiceImpl extends ServiceImpl<AppMapper, AppEntity> implements AppService {
  ...
  public AppServiceImpl(
      AppVersionMapper appVersionMapper, CacheManager cacheManager, ReferService referService) {
  ...
  public void publishApp(String appId) {
    ...
    Integer referType =
        app.getType() == AppType.BASIC
            ? ReferTypeEnum.MAIN_TYPE_AGENT.getType()
  ...
  public PagingList<ApplicationVersion> listAppVersions(AppQuery query) {
    ...
    return new PagingList<>(
        query.getCurrent(), query.getSize(), pageResult.getTotal(), appVersions);
  }

  /**
   * Gets specific version of an application. Supports 'latest' and 'lastPublished' special version
   * IDs
   *
   ...
   */
  @Override
  public PagingList<Application> getApplicationPublishedAndNotComponentList(
    ...
    applicationEntities.forEach(
        applicationEntity -> applicationDTOS.add(toApplicationDTO(applicationEntity)));
    return new PagingList<>(
        request.getCurrent(), request.getSize(), pageResult.getTotal(), applicationDTOS);
  }
  ...
  public String copyApp(String appId) {
    ...
    cacheManager.put(key, newEntity);
  ...
  private AppEntity getAppById(String workspaceId, String appId) {
    ...
    AppEntity entity = cacheManager.get(key);
    if (entity != null) {
    ...
    versionQueryWrapper
        .eq(AppVersionEntity::getWorkspaceId, workspaceId)
        .eq(AppVersionEntity::getAppId, appId)
