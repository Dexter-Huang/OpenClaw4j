...
/**
 * REST controller for managing agent applications. Handles CRUD operations, version management, and
...
public class AppController {
   ...
   */
  @PutMapping("/{appId}")
  public Result<String> updateApp(
      @PathVariable("appId") String appId, @RequestBody Application app) {
    RequestContext context = RequestContextHolder.getRequestContext();
  ...
  public Result<Void> publishApp(@PathVariable("appId") String appId) {
    ...
    if (app.getType() == AppType.WORKFLOW) {
      ...
      WorkflowConfig appOrchestraConfig =
          JsonUtils.fromJson(applicationVersion.getConfig(), WorkflowConfig.class);
      ExecuteProcessor.CheckFlowParamResult checkFlowParamResult =
          workflowExecuteManager.checkWorkflowConfig(appOrchestraConfig);
      if (!checkFlowParamResult.isSuccess()) {
        StringBuilder stringBuilder = new StringBuilder();
        checkFlowParamResult
            .getCheckNodeParamResults()
            .forEach(
                nodeParamResult ->
                    stringBuilder
                        .append("Node【")
  ...
  public Result<PagingList<ApplicationVersion>> listAppVersions(
      @PathVariable("appId") String appId, @ApiModelAttribute AppQuery query) {
  ...
  public Result<ApplicationVersion> getApplication(
