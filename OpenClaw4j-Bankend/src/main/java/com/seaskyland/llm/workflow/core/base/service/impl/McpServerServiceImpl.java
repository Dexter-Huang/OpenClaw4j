...
import static com.seaskyland.llm.workflow.core.base.constants.CacheConstants.CACHE_APP_WORKSPACE_ID_PREFIX;
...
import static com.seaskyland.llm.workflow.core.utils.concurrent.ThreadPoolUtils.TOOL_TASK_EXECUTOR;
import static com.seaskyland.llm.workflow.runtime.enums.ErrorCode.GET_TOOLS_ERROR;
import static com.seaskyland.llm.workflow.runtime.enums.ErrorCode.MCP_NOT_FOUND;
import static com.seaskyland.llm.workflow.runtime.enums.ErrorCode.TOOL_EXECUTION_ERROR;
import static com.seaskyland.llm.workflow.runtime.enums.ErrorCode.TOOL_PARAMS_MISSING;
...
import com.seaskyland.llm.workflow.core.base.manager.MCPManager;
...
import com.seaskyland.llm.workflow.runtime.domain.mcp.McpServerGetToolsRequest;
...
import com.seaskyland.llm.workflow.runtime.enums.McpServerTypeEnum;
...
/**
 * Implementation of MCP (Model Context Protocol) Server Service Handles CRUD operations and tool
...
public class McpServerServiceImpl extends ServiceImpl<McpServerMapper, McpServerEntity>
  ...
  public McpServerServiceImpl(MCPManager mcpManager, CacheManager cacheManager) {
  ...
  public String createMcp(McpServerDetail detail) {
    try {
      ...
      Result<String> checkResult =
          mcpManager.processInstallConfig(detail.getDeployConfig(), detail.getInstallType());
      ...
      entity.setType(
          detail.getType() == null ? McpServerTypeEnum.CUSTOMER.name() : detail.getType());
      this.save(entity);
      return serverCode;
    } catch (Exception e) {
  ...
  public McpServerEntity getMcpByCode(String workspaceId, String serverCode, Integer status) {
    ...
    McpServerEntity entity = cacheManager.get(key);
    if (entity != null) {
    ...
    LambdaQueryWrapper<McpServerEntity> queryWrapper = new LambdaQueryWrapper<>();
    queryWrapper
        .eq(McpServerEntity::getServerCode, serverCode)
        .eq(McpServerEntity::getWorkspaceId, workspaceId);
    if (status != null) {
  ...
  public Result<List<McpTool>> getTools(McpServerGetToolsRequest request) {
    try {
      McpServerEntity entity =
          getMcpByCode(request.getWorkspaceId(), request.getServerCode(), null);
      if (entity == null) {
  ...
  public Result<McpServerCallToolResponse> callTool(McpServerCallToolRequest request) {
    try {
      if (StringUtils.isBlank(request.getServerCode())
          || StringUtils.isBlank(request.getToolName())
