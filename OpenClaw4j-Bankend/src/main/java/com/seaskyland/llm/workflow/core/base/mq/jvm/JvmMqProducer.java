 ...
 *
 * <p>All three send methods (sync, async, delayed) are fully supported without
...
public class JvmMqProducer implements MqProducer {
	...
	/**
	 * Publishes the message synchronously and returns after all subscribers have been
	 * notified.
	 */
	@Override
	...
	/**
	 * Publishes the message asynchronously; returns immediately without waiting for
	 * delivery. The supplied {@code callback} is invoked upon completion or failure.
	 */
	@Override
	public void sendAsync(MqMessage message, SendCallback callback) {
		ensureMessageId(message);
		log.debug("[JvmMQ] sendAsync topic='{}' messageId='{}'", message.getTopic(), message.getMessageId());
	...
	public SendResult sendDelay(MqMessage message, int delaySeconds) {
		ensureMessageId(message);
		log.debug("[JvmMQ] sendDelay topic='{}' delay={}s messageId='{}'",
				message.getTopic(), delaySeconds, message.getMessageId());
