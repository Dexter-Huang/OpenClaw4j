...
import com.seaskyland.llm.workflow.core.base.manager.HttpClientManager;
...
import com.seaskyland.llm.workflow.runtime.domain.workflow.inner.TimeoutConfig;
 ...
 * API Node Processor
 *
 * <p>This processor is responsible for executing API calls within workflows, supporting various
 * HTTP methods, authentication types, and request/response formats.
 *
 * <p>Key Features: 1. Supports multiple HTTP methods (GET, POST, PUT, DELETE, etc.) 2. Handles
 * various authentication types (Bearer, API Key) 3. Supports different request body formats (JSON,
 * form-data) 4. Manages request headers and query parameters 5. Handles response parsing and
 * formatting 6. Supports timeout and retry configurations 7. Provides error handling and validation
 * 8. Supports template variable substitution in requests
 *
 * @author guning.lt
 * @version 1.0.0-M1
 */
...
public class APIExecuteProcessor extends AbstractExecuteProcessor {
  ...
  public NodeResult innerExecute(
    ...
    RpcResult result =
        httpClientManager.doApiNodeWithRequestId(
            HttpClientManager.Method.valueOf(method.toUpperCase()),
    ...
    if (result != null && result.isSuccess() && result.getCode() == 200) {
      ...
      log.error(
          "invoke api failed, requestId={},flowConfig={},result={}",
          context.getRequestId(),
          node.getConfig(),
      ...
      if (result != null) {
        ...
        nodeResult.setError(
            ErrorCode.WORKFLOW_EXECUTE_ERROR.toError("api node exception info: response is null"));
      }
    }
    return nodeResult;
  }
  ...
  private void handleAuthorization(
    ...
    if (authorization != null) {
      String authType =
          StringUtils.isBlank(authorization.getAuthType())
              ? Authorization.AuthTypeEnum.NoAuth.name()
      ...
      if (Authorization.AuthTypeEnum.BearerAuth.name().equals(authType)) {
        ...
        String addTo =
            StringUtils.isBlank(authorization.getAuthConfig().getAddTo())
                ? Authorization.AddToEnum.Header.name()
                : authorization.getAuthConfig().getAddTo();
   ...
   */
  private List<Map<String, Object>> buildParams(
      List<Node.InputParam> paramsArray, WorkflowContext context) {
    ...
    paramsArray.stream()
        .forEach(
            param -> {
              Object valueFromRequestContext =
                  VariableUtils.getValueStringFromContext(param, context);
              Map<String, Object> paramNew = new HashMap<>();
   ...
   */
  private Map<String, Object> buildBodyFormParams(
      Map<String, Object> body, WorkflowContext context) {
    ...
    if ("form-data".equalsIgnoreCase(type)) {
      ...
      List<Node.InputParam> data =
          JsonUtils.fromJsonToList(JsonUtils.toJson(o), Node.InputParam.class);
      data.forEach(
          param -> {
            Object valueFromRequestContext =
                VariableUtils.getValueStringFromContext(param, context);
      ...
      if (!validJSON) {
        log.info(
            "log used for query jsonFormatData:{} ,requestID:{}", data, context.getRequestId());
  ...
  private String constructData(
      Set<String> keys, Map<String, Object> localVariableMap, String data) {
    for (String key : keys) {
      ...
      if (o == null) {
        ...
        if (o instanceof Map || o instanceof List) {
          data =
              data.replaceAll("\\$\\{" + key + "}", Matcher.quoteReplacement(JsonUtils.toJson(o)));
        } else {
  ...
  private Map<String, Object> buildHeaders(
      List<Node.InputParam> headersArray, WorkflowContext context) {
    ...
    headersArray.stream()
        .forEach(
            header -> {
              Object valueFromRequestContext =
                  VariableUtils.getValueStringFromContext(header, context);
  ...
  public CheckNodeParamResult checkNodeParam(DirectedAcyclicGraph<String, Edge> graph, Node node) {
    ...
    if (!"post".equalsIgnoreCase(config.getMethod())
        && !"get".equalsIgnoreCase(config.getMethod())
  ...
  private Map<String, Object> convertToInputParams(
      String method,
      String url,
