...
import com.seaskyland.llm.workflow.core.base.service.AppComponentService;
...
import com.seaskyland.llm.workflow.core.workflow.processor.impl.EndExecuteProcessor;
...
import com.seaskyland.llm.workflow.runtime.domain.component.AppComponentConfig;
...
import com.seaskyland.llm.workflow.runtime.enums.AppComponentStatusEnum;
...
/**
 * Controller for managing application components in the SAA Studio platform. This controller
 * provides endpoints for: 1. CRUD operations on application components 2. Component configuration
 * management 3. Component reference relationship queries 4. Component schema queries 5. Publishable
 * application queries
 *
 * <p>The component system allows applications to be published as reusable components that can be
 * referenced by other applications, enabling modular application development.
 *
 * @author guning.lt
 * @since 1.0.0.3
 */
...
public class AppComponentController {
   ...
   *
   * @param request Query parameters for filtering components including: - type: Component type -
   *     name: Component name - appId: Associated application ID - status: Component status -
   *     pageSize: Number of items per page - pageNum: Page number
   * @return Result containing a PagingList of AppComponent objects
   */
  @GetMapping()
  public Result<PagingList<AppComponent>> getAppComponentPageList(
      @ApiModelAttribute AppComponentQuery request) {
  ...
  /**
   * Retrieves a paginated list of applications that can be published as components. This endpoint
   * filters out applications that are already published as components.
   *
   * @param request Query parameters including: - type: Application type - appName: Application name
   * @return Result containing a PagingList of Application objects that can be published as
   *     components
   */
  @GetMapping("/app-publishable")
  public Result<PagingList<Application>> getAppPublishablePageList(
      @ApiModelAttribute AppComponentQuery request) {
    RequestContext context = RequestContextHolder.getRequestContext();
    ...
    List<AppComponent> appComponentList =
        appComponentService.getAppComponentListAll(
            request.getType(), AppComponentStatusEnum.Published.getCode());
    ...
    List<Long> ids =
        appService.getApplicationPublished(request.getType(), request.getAppName(), codes);

    PagingList<Application> componentList =
        appService.getApplicationPublishedAndNotComponentList(request, codes, ids);
    return Result.success(context.getRequestId(), componentList);
  }

  /**
   * Publishes a new application component. Creates a new component from an existing application
   * with specified configuration.
   *
   * @param request Component details including: - type: Component type - name: Component name -
   *     config: Component configuration - appId: Source application ID - description: Component
   *     description
  ...
  /**
   * Updates an existing application component. Modifies the configuration and details of a
   * published component.
   *
   * @param code Unique code of the component to update
   * @param request Updated component details including: - type: Component type - name: Component
   *     name - config: Updated configuration - appId: Associated application ID - description:
   *     Updated description
   * @return Result indicating successful update
   */
  @PutMapping("/{code}")
  public Result<String> updateComponent(
      @PathVariable("code") String code, @RequestBody AppComponentQuery request) {
    RequestContext context = RequestContextHolder.getRequestContext();
  ...
  /**
   * Deletes an application component. Removes a component from the system by its unique code.
   *
  ...
  /**
   * Retrieves detailed information about a component by its unique code. Includes merged
   * configuration from both component and source application.
   *
   * @param code Unique code of the component
   * @return Result containing detailed AppComponent information with merged configuration
   */
  ...
  public Result<AppComponent> detailByCode(@PathVariable("code") String code) {
    ...
    try {
      AppComponent appComponentByCode =
          appComponentService.getAppComponentByCode(
              code, AppComponentStatusEnum.Published.getCode());
      ...
      AppComponentConfig appComponentConfig =
          appComponentManager.getAppComponentInputConfig(appComponentByCode);
      ...
      AppComponentConfig applicationInputConfig =
          appComponentManager.getApplicationInputConfig(application, false);
      // merge component config
      applicationInputConfig =
          appComponentManager.mergeConfig(applicationInputConfig, appComponentConfig);
  ...
  /**
   * Retrieves detailed information about a component by its application ID. Includes merged
   * configuration from both component and source application.
   *
  ...
  public Result<AppComponent> detailByAppId(@PathVariable("appId") String appId) {
    ...
    try {
      AppComponent appComponentByAppId =
          appComponentService.getAppComponentByAppId(
              appId, AppComponentStatusEnum.Published.getCode());
      if (appComponentByAppId == null) {
        return Result.success(context.getRequestId(), null);
      }
      // get component inputScheme
      AppComponentConfig appComponentConfig =
          appComponentManager.getAppComponentInputConfig(appComponentByAppId);
   ...
   * @return Result containing a list of components that reference the specified component
   */
  ...
  public Result<AppComponent> queryConfig(@PathVariable("appId") String appId) {
    ...
    try {
      ...
      AppComponentConfig applicationInputConfig =
          appComponentManager.getApplicationInputConfig(application, true);
  ...
  public Result<List<AppComponent>> listByCodes(@RequestBody AppComponentQuery request) {
    ...
    try {
      List<AppComponent> appComponentListByCodes =
          appComponentService.getAppComponentListByCodes(request.getCodes());
  ...
  /**
   * Retrieves the schema of a component by its unique code. The schema includes input/output
   * parameters and output type information.
   *
   * @param code Unique code of the component
   * @return Result containing a map of schema information including: - input: Input parameter
   *     definitions - output: Output parameter definitions - output_type: Type of output
   *     (text/json)
   */
  ...
  public Result<Map<String, Object>> getComponentSchemaByCodes(
      @RequestBody AppComponentQuery request) {
  ...
  private Map<String, Object> getSchemas(String code) {
    AppComponent appComponentByCode =
        appComponentService.getAppComponentByCode(code, AppComponentStatusEnum.Published.getCode());
    Map<String, List<AppComponentConfig.Params>> stringListMap =
        appComponentManager.fetchInputAndOutputParams(appComponentByCode);
    ...
    if (appComponentByCode.getType().equals(AppType.WORKFLOW.getValue())) {
      ApplicationVersion appVersion =
          appService.getAppVersion(appComponentByCode.getAppId(), "lastPublished");
      if (appVersion == null) {
        throw new BizException(ErrorCode.APP_NOT_FOUND.toError());
      }
      WorkflowConfig appOrchestraConfig =
          JsonUtils.fromJson(appVersion.getConfig(), WorkflowConfig.class);
      List<Node> nodes =
          appOrchestraConfig.getNodes().stream()
              .filter(nodeTmp -> nodeTmp.getType().equals(NodeTypeEnum.END.getCode()))
              .toList();
      Node endNode = nodes.get(0);
      EndExecuteProcessor.NodeParam endNodeParam =
          JsonUtils.fromMap(
              endNode.getConfig().getNodeParam(), EndExecuteProcessor.NodeParam.class);
