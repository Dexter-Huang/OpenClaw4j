 ...
 * <p>Features:
 * <ul>
 *   <li>TTL support – entries are lazily evicted on access and proactively cleaned every
 *       60 seconds by a background daemon thread.</li>
 *   <li>Atomic counters – using {@link AtomicLong} per key.</li>
 *   <li>Set operations – using {@link ConcurrentHashMap#newKeySet()} per key.</li>
 *   <li>Map operations – nested {@link ConcurrentHashMap} per key.</li>
 *   <li>Lock operations – TTL-based string locks ({@code lock}/{@code unlock}) and
 *       {@link ReentrantLock}-based blocking locks ({@code lockWithRedisson}/{@code unlockRedisson}).</li>
 * </ul>
 *
 * <p><strong>Note:</strong> This implementation is intended for single-node deployments
 * or development/testing scenarios. It does <em>not</em> support distributed locking
...
public class JvmCacheStore implements CacheStore {
	...
	private final ConcurrentHashMap<String, ConcurrentHashMap<String, Object>> mapCaches = new ConcurrentHashMap<>();
	...
	public <V> void put(String key, V value, Duration duration) {
		long expiryMs = (duration == null || duration.isZero() || duration.isNegative())
				? 0L : System.currentTimeMillis() + duration.toMillis();
	...
	/**
	 * Simple TTL-based lock backed by a {@link ConcurrentHashMap}. Uses compare-and-swap
	 * semantics to detect and recover from expired (dead) locks.
	 */
	@Override
	 ...
	 *
	 * <p><strong>Note:</strong> The {@code lockDuration} parameter is accepted for API
	 * compatibility but is NOT enforced in the JVM implementation – the lock is held until
	 * {@link #unlockRedisson(String)} is explicitly called (or the JVM exits).
	 */
	@Override
