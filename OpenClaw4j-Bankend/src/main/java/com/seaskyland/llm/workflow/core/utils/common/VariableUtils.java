...
public class VariableUtils {
  ...
  public static String getValueStringFromPayload(String expression, Map<String, Object> payload) {
    ...
    if (valueFromContext instanceof String
        || valueFromContext instanceof Number
  ...
  public static boolean setValueForPayload(
      String expression, Map<String, Object> payload, Object value) {
    if (StringUtils.isBlank(expression) || payload == null) {
      return false;
    }
    try {
  ...
  public static String getValueStringFromContext(
      Node.InputParam inputParam, WorkflowContext context) {
    ...
    try {
      ...
      throw new BizException(
          ErrorCode.INVALID_PARAMS.toError(
              key, "expected type is " + type + " but not match the current value type."));
    }
    throw new BizException(
        ErrorCode.INVALID_PARAMS.toError(
            key, "expected type is " + type + " but not match the current value type."));
